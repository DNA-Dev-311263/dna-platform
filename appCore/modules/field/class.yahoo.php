...
class CField_Yahoo extends Field_Contact
{
    /**
    ...
    public function create($back)
    {
        ...
        if (isset($_POST['save_field_' . $this->getFieldType()])) {
            ...
            if (!isset($_POST['new_yahoo'][$mand_lang])) {
                $out->add(
            ...
            if ($_POST['new_yahoo'][$mand_lang] == $lang->def('_FIELD_NAME') || trim($_POST['new_yahoo'][$mand_lang]) == '') {
                $out->add(
                    getErrorUi($lang->def('_ERR_MUST_DEF_MANADATORY_TRANSLATION'))
                    . getBackUi($this->getUrl() . '&amp;type_field='
                        . $this->getFieldType() . '&amp;back=' . $back_coded, $std_lang->def('_BACK')),
			...
			('" . $this->getFieldType() . "', '" . $mand_lang . "', '" . $_POST['new_yahoo'][$mand_lang] . "', '" . $show_on . "', '" . $use_multilang . "') ")) {
            ...
            $re = true;
            //insert other field
            foreach ($_POST['new_yahoo'] as $lang_code => $translation) {
        ...
        $out->add(
            $form->getFormHeader($lang->def('_NEW_YAHOO'))
    ...
    public function getIMBrowserImageSrc($id_user, $field_value)
    {
        ...
        return 'http://opi.yahoo.com/online?u=' . $field_value . '&m=g&t=5&l=us';
    }
}
