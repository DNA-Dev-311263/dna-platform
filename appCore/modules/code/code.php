...
function assignCourse()
{
    ...
    $selector = new Course_Manager();
    $selector->show_catalogue_selector = false;
    ...
    if (FormaLms\lib\Get::req('confirm', DOTY_MIXED, '')) {
        ...
        $course_selected = $selector->getCourseSelection($_POST);
        $coursepath_selected = $selector->getCoursePathSelection($_POST);
        
        if ($code_manager->insertCourseAssociation($course_selected, $id_code_group) && 
			$code_manager->insertCoursepathAssociation($coursepath_selected, $id_code_group)) {
            Util::jump_to('index.php?modname=code&amp;op=list&result=ok');
        }
        Util::jump_to('index.php?modname=code&amp;op=list&result=err');
    }
    ...
    $array_coursepath_associated = $code_manager->getCoursepathAssociated($id_code_group);

    $selector->resetCourseSelection($array_course_associated);
    $selector->resetCoursePathSelection($array_coursepath_associated);
 
	$out = 
	getTitleArea(['index.php?modname=code&amp;op=list' => $lang->def('_CODE'), $lang->def('_ASSIGN_COURSES', 'code'), ])
	. '<div class="std_block">'
	. Form::openForm('add_group_code_form', 'index.php?modname=code&amp;op=assign_course&amp;id_code_group=' . $id_code_group)

	. $selector->loadSelector(true, true)
