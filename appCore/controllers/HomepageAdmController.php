...
class HomepageAdmController extends AdmController
{
    ...
    public function show()
    {
        ...
        $params['isRequestFirstPwd'] = $this->model->isRequestFirstPwd(); 				//ABR
    ...
    /**
     * Controlla e sistema il lo userID inserito dall'utente
     * funzione necessaria per installazione ALLIANZ
     * (ABR)
     */
	public function checkUserid($userid) {
		
		$page_title = Formalms\lib\Get::sett('page_title');
		$retVal = trim($userid);
		
		if (strpos($page_title, 'ARLA') !== false) {
			
			$strL2 = strtoupper(substr($retVal,0,2));
			
			if ($strL2 == "RU" && is_numeric(substr($retVal, 2)))
	...
	/**
	 * Gestisce la richiesta delle credenziali di primo accesso
	 * (ABR)
	 */
    public function firstpwd()
    {
		...
		$email 	= stripslashes( Formalms\lib\Get::req("email", DOTY_STRING) );

        if (!empty($_POST)) {
			// Se si arriva dal submit del form
			// Controllo se il formato del codice utente è corretto
			$userid = $this->checkUserid($userid);
			
			// Invio la password
			$res = $this->model->sendFirstPwd($userid, $email);

        }

		// Gestione messaggio esito
        switch ($res) {
            case USER_NOT_FOUND:
                $pos_err = 1;
            ...
            case EMAIL_NOT_FOUND:
                $pos_err = 2;
                $errorMessage = Lang::t("_EMAIL_NOT_FOUND", "register");
                break;
                
            case USER_ALREADY_LOGIN:
                $pos_err = 3;
                $errorMessage = Lang::t("_USER_ALREADY_LOGIN", "register");
                break;
                
            case FAILURE_GEN_FIRST_PWD:
                $pos_err = 3;
                $errorMessage = Lang::t("_FAILURE_GEN_FIRST_PWD", "register");
                break;           
                
            case FAILURE_SEND_FIRST_PWD:
                $pos_err = 3;
                $errorMessage = Lang::t("_OPERATION_FAILURE", "register");
                break;
                
            case SUCCESS_SEND_FIRST_PWD:
                $dataView['loginAction'] = Formalms\lib\Get::rel_path('base') . '/index.php?r=' . _login_;
                $dataView['message'] = Lang::t('_MAIL_SEND_SUCCESSFUL_FIRST_PWD', 'register');
        ...
        $frm .= '<div class="homepage__row homepage__row--gray homepage__row--form row-fluid">';
        $frm .= 	Form::openForm("first_pwd", Formalms\lib\Get::rel_path("base") . "/index.php?r=" . _request_first_pwd_);
        $frm .= 		'<div class="col-xs-12 col-sm-12">';
        
        $frm .=  			Form::getInputTextfield(
								'form-control ' . (($pos_err == 1) ? 'has-error' : ''),
								'user_userid',
								'userid',
								 $userid,
        ...
        if (($pos_err == 1))  $frm .= '<small class="form-text">* ' . $errorMessage . '</small>';

        $frm .= 			'<br/>';
        $frm .=  			Form::getInputTextfield(
								'form-control ' . (($pos_err == 2) ? 'has-error' : ''),
								'user_email',
								'email',
								 $email,
        ...
        if (($pos_err == 2))  $frm .= '<small class="form-text">* ' . $errorMessage . '</small>';

        $frm .= 		'</div>';
        
        $frm .= 		'<div class="col-xs-12 col-sm-12">';
        $frm .= 			Form::getButton('first_pwd_send', 'send', Lang::t("_SEND", "register"), 'forma-button forma-button--black thin');
		$frm .= 		'</div>';
        $frm .= 	Form::closeForm();
            
        if (($pos_err == 3))  {
				// Messaggio errore: utente già autenticato
				$link_lostpwd = Formalms\lib\Get::rel_path("base") . "/index.php?r=" . _lostpwd_;
				$frm .= '<div class="firstpwd_err">' . str_replace('[link]', $link_lostpwd, $errorMessage) . '</div>';
		}	
		$frm .= '</div>';
        ...
        $params['firstpwd'] = [
            'title' => Lang::t("_FIRST_PWD_TITLE", "register"),
            'istruction' => Lang::t("_FIRST_PWD_INSTRUCTION", "register"),
            'form' => $frm
        ];


		// Apertura view
        $this->render("firstpwd", $params);
    }
    ...
    public function lostPwd()
    {
        ...
        switch ($action) {
            case 'lost_user':
                ...
                //ABR: se non trovo l'utente con l'email significa che l'email non esiste
                if ($res == USER_NOT_FOUND) $res = EMAIL_NOT_FOUND; 
