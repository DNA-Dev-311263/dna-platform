...
class UsermanagementAdmController extends AdmController
{
    ...
    public function show()
    {
		...
		//mod. ABR (introdotto 'privacy_policy')
        $f_list = [
            ...
            'privacy_policy' 	=> Lang::t('_PRIVACY', 'standard'),
        ];
        ...
        switch ($res) {
            ...
            case 'ok_suspended': 
				$message = getResultUi(Lang::t('_OPERATION_SUCCESSFUL', 'standard')); 
				break; //ABR, feedback op. sospensione
        ...
        if (is_array($list) && count($list) > 0) {
            ...
            foreach ($list as $idst => $record) {
                $record_row = [
                    ...
                    'su'  => (Docebo::user()->getUserLevelId() == ADMIN_GROUP_GODADMIN && $idst != $current_user)
                        ? 'index.php?r=adm/impersonate/start&idst=' . (int) $idst
                        : false,
                ];
	...
	public function suspendByFile() {
		//>> ABR: Sospende gli utenti da un file caricato
		
		$acl_man = Docebo::user()->getAclManager();
		$step = FormaLms\Lib\Get::req('step', DOTY_INT, 0);
		$base_url = 'index.php?r='.$this->link.'/';
		
		// Controllo permessi modifica e azione annulla
		if (!$this->permissions['mod_user'] || isset($_POST['undo'])) Util::jump_to($base_url);
		
		
		// Recupero setting intestazioni file
		$param_name = "userman_fileupdate_susp_header";
		$param_exp = Get::sett($param_name);
		$headers  = explode("=", $param_exp);
		...
		// Preparo il messaggio di avvertimento
		$warning_desc = Lang::t('_WARNING_SUSPEND', 'user_management');
		$warning_desc = str_replace('['.$headers[1].']', $headers[0], $warning_desc);
		
		
		if($step == 0) {
			// Apertura view
			
			$err = Get::req('err', DOTY_STRING, "");
			$error_message = "";

			// Preparo parametri
			$params = array('warning_desc' => $warning_desc, 'error_message' => $message);
		
			// Apro la vista
			$this->render('suspend_users.php', $params);
			...
			if (!$file['name']){
				...
				// Manca il file: apro vista con errore
				$params = array('warning_desc' => $warning_desc, 'error_message' => getErrorUi(Lang::t('_NO_FILE', 'user_managment')));
				$this->render('suspend_users.php', $params);
				...
				if ( $xlsx = SimpleXLSX::parse($file['tmp_name']) ){
					...
					foreach ($xlsx->rows() as $r => $row){
						if ($r == 0){	
							...
							// Trovo campo userid
							for ($c = 0; $c < $cols; $c ++)
								if($row[$c] == $headers[0])	$k = $c;

						} else {
							// Esco se non trovo il campo userid
							if ($k == -1)  break;
							
							// Recupero id utente
							$idst = $acl_man->getUserST(trim($row[$k]));
							
							if($idst) $arr_data[] = $idst;
						}						
					}
				...
				if (count($arr_data) == 0) {
					// Mancano dati validi: apro vista con errore
					$params = array('warning_desc' => $warning_desc, 'error_message' => getErrorUi(Lang::t('_NO_DATA', 'standard')));
					$this->render('suspend_users.php', $params);					
					
				} else {
					// Aggiorno e torno alla vista principale			
					$res = $this->model->suspendUsers($arr_data);
					
					if($res) Util::jump_to($base_url.'&res=ok_suspended&n='.$res);
				}	
			}
		}
	}
