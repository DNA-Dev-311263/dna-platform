...
class MyFriends
{
    ...
    public function _getFriendsTable()
    {
        return $GLOBALS['prefix_fw'] . '_user_friend';
    }
     ...
     */
    public function getFriendsList($limit = false, $random_order = false, $waiting = '0', $only_id = false)
    {
        $query = '
        ...
        if ($waiting !== false) {
            $query .= ' AND ' . $this->arr_field[MYFRIEND_WAITING] . " = '" . $waiting . "'";
        }
        ...
        while (list($id, $waiting) = $this->fetch_row($re_query)) {
            if ($waiting === false || $only_id === true) {
                $friends[$id] = $id;
            } else {
                $friends[$id] = ['id' => $id, 'waiting' => $waiting];
            }
        }
     ...
     */
    public function getAllFriendsSubdivided()
    {
        $query = '
		SELECT ' . $this->arr_field[MYFRIEND_ID_FRIEND] . ', ' . $this->arr_field[MYFRIEND_WAITING] . ' 
		FROM ' . $this->_getFriendsTable() . '
        ...
        while (list($id, $waiting) = $this->fetch_row($re_query)) {
            if ($waiting) {
                $friends['waiting'][$id] = $id;
            } else {
                $friends['effective'][$id] = $id;
            }
        }
     ...
     * @return array the list of the idst of the user 's friends in 'effective' and the waiting in 'waiting'
     */
    public function getAllFriendsSubdividedForUsers($arr_user)
    {
        $query = '
        ...
        while (list($id_user, $friend, $waiting) = $this->fetch_row($re_query)) {
            if ($waiting) {
                $friends[$id_user]['waiting'][$friend] = $friend;
            } else {
                $friends[$id_user]['effective'][$friend] = $friend;
            }
        }
    ...
    public function getFriendsCount($waiting = '0')
    {
        $query = '
		SELECT COUNT(*) 
		FROM ' . $this->_getFriendsTable() . '
    ...
    */
    public function controlPendentRequest()
    {
        $controlled_id = [];
        $control_query = '
     ...
     */
    public function getPendentRequest()
    {
        $query = '
     ...
     */
    public function getPendentRequestCount()
    {
        $query = '
		SELECT COUNT(*)
		FROM ' . $this->_getFriendsTable() . '
    ...
    public function approveFriend($id_friend)
    {
        $query = '
		UPDATE ' . $this->_getFriendsTable() . '
		SET ' . $this->arr_field[MYFRIEND_WAITING] . " = '" . MF_APPROVED . "',
    ...
    public function refuseFriend($id_friend)
    {
        $query = '
     ...
     * @return bool true if the user is a friend, false otherwise
     */
    public function isFriend($id_friend, $also_waiting = false)
    {
        $query = '
		SELECT ' . $this->arr_field[MYFRIEND_ID_FRIEND] . ' 
		FROM ' . $this->_getFriendsTable() . '
        ...
        if ($also_waiting === false) {
            $query .= " AND waiting = '" . MF_APPROVED . "'";
        }
     ...
     */
    public function addFriend($id_friend, $waiting, $request_text)
    {
        if ($this->isFriend($id_friend)) {
            return true;
        }
     ...
     */
    public function delFriend($id_friend)
    {
        $query = '
		DELETE FROM ' . $this->_getFriendsTable() . '
		WHERE ' . $this->arr_field[MYFRIEND_ID_USER] . " = '" . $this->_id_user . "'
