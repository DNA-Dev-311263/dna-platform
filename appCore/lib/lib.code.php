...
class CodeManager
{
    ...
    //ABR
    protected function _getCodeCoursepathTable()
    {
        return $GLOBALS['prefix_fw'] . '_code_coursepath';
    }
    ...
    //ABR
    public function getCoursepathAssociated($id_code_group)
    {
        $query = 'SELECT idPath'
                    . ' FROM ' . $this->_getCodeCoursepathTable()
                    . " WHERE idCodeGroup = '" . $id_code_group . "'";
    ...
    protected function clearCourseAssociation($id_code_group)
    {
 	...
 	//ABR
    protected function clearCoursepathAssociation($id_code_group)
    {
        $query = 'DELETE'
                    . ' FROM ' . $this->_getCodeCoursepathTable()
                    . " WHERE idCodeGroup = '" . $id_code_group . "'";
	...
	//ABR
    public function insertCoursepathAssociation($coursepath_selected, $id_code_group)
    {
        $this->clearCoursepathAssociation($id_code_group);

        if (!count($coursepath_selected)) {
            return true;
        }

        $first = true;

        $query = 'INSERT INTO ' . $this->_getCodeCoursepathTable()
                    . ' (idCodeGroup, idPath)'
                    . ' VALUES ';

        foreach ($coursepath_selected as $id_path) {
            if ($first) {
                $query .= "('" . $id_code_group . "', '" . $id_path . "')";
                $first = false;
            } else {
                $query .= ", ('" . $id_code_group . "', '" . $id_path . "')";
            }
    ...
    //ABR
    public function getCoursepathAssociateWithCode($code)
    {
        $id_code_group = $this->getGroupOfCode($code);

        return $this->getCoursepathAssociated($id_code_group);
    }
    ...
    public function resetUserCode($user_idst)
    {
        $query = 'UPDATE ' . $this->_getCodeTable()
                    . ' SET `idUser` = null,'
                    . " used = '0'"
    ...
    public function getTotalCodeNumber()
    {
        $query = 'SELECT COUNT(*)'
                . ' FROM ' . $this->_getCodeTable();
    ...
    public function getCodeNumberForTypeInGroup($id_code_group)
    {
    ...
    public function getGroupCodeByUse($id_code_group, $unlimited, $used, $not_used)
    {
        if ($used && $not_used) {
            $used_q = '0, 1';
        } elseif ($used) {
