...
class DoceboUserNotifier extends DoceboEventConsumer
{
    ...
    public function actionEvent(&$event) {
		//Mod. ABR: Recupero dalla tabella delle proprietà evento anche cc, bcc
        ...
        $arr_recipients = explode(',', $event->getProperty('recipientid'));
 		$recipients_cc 	    = $event->getProperty('recipientcc'); 					//ABR
		$recipients_bcc 	= $event->getProperty('recipientbcc'); 				    //ABR
    ...
    public function _sendMail($subject, $body, $attachments, &$mail_recipients, &$users_info = false, $mail_recipients_cc = false, $mail_recipients_bcc = false)
    //Mod. ABR
    //Inseriti argomenti cc e bcc per invio mail in copia (gli indirizzi devono essere passati in stringa separati da spazio).
    {
        $mailer = FormaMailer::getInstance();
        $acl_man = Docebo::user()->getAclManager();
        
        $isHtml = ($body != strip_tags($body) ? true : false); //ABR
        
		$mail_recipients_cc  = (is_string($mail_recipients_cc ) && strlen($mail_recipients_cc) > 0 ? $mail_recipients_cc  : null); 		//ABR
		$mail_recipients_bcc = (is_string($mail_recipients_bcc) && strlen($mail_recipients_bcc) > 0  ? $mail_recipients_bcc : null); 	//ABR
        ...
        foreach ($mail_recipients as $id => $mail) {
			...
			$mailer->SendMail(
				...
				array(
					...
					MAIL_RECIPIENTSCC  => $mail_recipients_cc,
					MAIL_RECIPIENTSBCC => $mail_recipients_bcc,
					MAIL_HTML => $isHtml,
					MAIL_QUEUE_INFO => array('caller' => get_class($this))
				)
			);
