<div class="std_block">
		...
		<?php if (!$under_maintenence) { ?>
			...
			<div class="login-box">
				<?php if (!$block_attempts) { ?>
					...
					<?php foreach ($this->model->getLoginGUI() as $loginGUI) {
    echo $loginGUI;
} ?>
					<?php if ($done) { ?>
						<div>
	...
	<div class="footer">
		<div id="link">
			<?php if($this->model->isRequestFirstPwd()) { ?>
				<a class="first" href="<?php echo FormaLms\lib\Get::rel_path('base') . '/index.php?r=' . _request_first_pwd_; ?>"><?php echo Lang::t('_REQUEST_FIRST_PWD', 'login'); ?></a>
			<?php } ?>
