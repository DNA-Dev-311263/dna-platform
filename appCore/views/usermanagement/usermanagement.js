...
var UserManagement = {
		...
		YAHOO.util.Event.onDOMReady(function() {
			...
			var createUserCallback = function(o) {
				//this.destroy();
					...
					var panel = D.get("create_user_tab1");
					
					$(panel).find('input:text, input:password, input:file, textarea')
							.each(function() {
								$(this).val('');
							});
           ...
           var suspendByFileEvent = function() {
                //ABR
                var url = "index.php?r="+U.baseUrl+"/suspendByFile&action=0";
			...
			if (U.perms.mod_user) {
                ...
                items.push({id:"opt6", text: L.get('_SUSPEND_BY_FILE'), onclick: { fn: suspendByFileEvent }}); //ABR
			}
			if (U.perms.del_user) {
				items.push({id:"opt7", text: L.get('_DEL'), onclick: { fn: multiDeleteEvent, obj: "ajax.adm_server.php?r="+U.baseUrl+"/delmultiuser" }});
			}
	...
	postRenderEvent: function() {
		...
		// "Accedi come" buttons — inject new <td> per row after YUI finishes rendering
		if (!document.getElementById('su-fa-css')) {
			var suFaLink = document.createElement('link');
			suFaLink.id = 'su-fa-css';
			suFaLink.rel = 'stylesheet';
			suFaLink.type = 'text/css';
			suFaLink.href = '/templates/standard/style/font-awesome/css/font-awesome.min.css';
			document.head.appendChild(suFaLink);
		}
		setTimeout(function() {
			var rs = DataTable_usertable.getRecordSet().getRecords();
			if (!rs.length) { return; }
			// Inject header th once
			if (!document.getElementById('su-th-col')) {
				var suFirstTr = DataTable_usertable.getTrEl(rs[0]);
				if (suFirstTr) {
					var suTable = suFirstTr.parentNode && suFirstTr.parentNode.parentNode;
					var suHeadRow = suTable ? suTable.querySelector('thead tr') : null;
					if (suHeadRow) {
						var suTh = document.createElement('th');
						suTh.id = 'su-th-col';
						suTh.style.cssText = 'width:28px;min-width:28px;';
						suHeadRow.appendChild(suTh);
					}
				}
			}
			for (var suIdx = 0; suIdx < rs.length; suIdx++) {
				var suUrl = rs[suIdx].getData('su');
				var suUserId = rs[suIdx].getData('id');
				if (document.getElementById('user_su_' + suUserId)) { continue; }
				var suTr = DataTable_usertable.getTrEl(rs[suIdx]);
				if (!suTr) { continue; }
				var suNewTd = document.createElement('td');
				suNewTd.style.cssText = 'width:28px;min-width:28px;text-align:center;vertical-align:middle;padding:2px 0;';
				if (suUrl) {
					var suA = document.createElement('a');
					suA.id = 'user_su_' + suUserId;
					suA.href = suUrl;
					suA.title = 'Accedi come questo utente';
					suA.style.cssText = 'color:#d97706;text-decoration:none;display:inline-block;';
					suA.innerHTML = '<i class="fa fa-user-secret" style="font-size:14px;"></i>';
					suNewTd.appendChild(suA);
				}
				suTr.appendChild(suNewTd);
			}
		}, 200);
