...
class HomepageAdm extends Model
{
    ...
    /**
     * Restituisce se è abilitato il link per il recupero password di primo accesso
     * (ABR)
     */
     public function isRequestFirstPwd() {
		return ($this->options->getOption("pass_first_request") == 'on');
	}
    ...
    /**
     * Invia una password iniziale all'utente che non ha mai avuto accesso alla piattaforma
     * (ABR)
     */
    public function sendFirstPwd($userid, $email) {
        
        // Recupero l'utente
        $acl_man =& Docebo::user()->getAclManager();
        $userid = $acl_man->absoluteId($userid);
        $user_info = $acl_man->getUser(false, $userid);
        
        // Eseguo controlli
        if(!$user_info) return USER_NOT_FOUND;
        
        
        if(strtolower($email) != strtolower($user_info[ACL_INFO_EMAIL])) return EMAIL_NOT_FOUND;
        
        
        if($user_info[ACL_INFO_LASTENTER]) return USER_ALREADY_LOGIN;
        
        // Genero password casuale e imposto dati di aggiornamento
        $new_pwd = $acl_man->random_password();
        $force_change = 1;
        $lastenter = false;
        
        // Aggiorno l'utente
        $res = $acl_man->updateUser($user_info[ACL_INFO_IDST], false, false, false, $new_pwd,  false, false, false, $lastenter, false, $force_change);
        
		if(!$res) {
			//Se aggiornamento password non riuscito
			return FAILURE_GEN_FIRST_PWD;
			...
			$sender         = $this->options->getOption('mail_sender');
			$recipients     = [ $user_info[ACL_INFO_EMAIL] ];
			$subject        = Lang::t("_FIRST_PWD_SUBJECT", "email");
			$params         = array(MAIL_SENDER_ACLNAME => false);
			$attachments    = false;
			
			$body           = Lang::t("_FIRST_PWD_HTML", "email", 
									array(
			...
			$res = $mailer->SendMail($sender, $recipients, $subject, $body, $attachments, $params);
			
			// Se invio non riuscito
			if(!$res) return FAILURE_SEND_FIRST_PWD;
		}
		
		// Risposta operazione riuscita
		return SUCCESS_SEND_FIRST_PWD;
    }
