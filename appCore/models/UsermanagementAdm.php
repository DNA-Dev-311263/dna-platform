...
class UsermanagementAdm extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->courseassn_man = new CourseassnManager();	 //ABR
    ...
    {
		//Mod. ABR per nuovo campo policy
            ...
            if (is_numeric($pagination['sort'])) {
                ...
                switch ($pagination['sort']) {
                    ...
                    case 'register_date':
                        ...
                        break;
					case 'privacy_policy': //ABR
						$sort = "u.privacy_policy"; break;
        ...
        if (isset($pagination['dir'])) {
            $dir = $this->clean_dir($pagination['dir']);
        }
        ...
        if ($is_subadmin && $is_dynfiltered) {
            ...
            if (count($t_arr) <= 0) {
                return [];
            } //return a 0-length array, because at this point the selection is void and no more query is necessary
            ...
            if (count($admin_info['users']) <= 0) {
                return [];
            } //if (count($admin_tree) <= 0) return array(); //return a 0-length array, because at this point the selection is void and no more query is necessary
        ...
        if ($idOrg != 0) {
            ...
            if ($is_subadmin) {
                $id_groups = array_intersect($id_groups, $admin_info['groups']);
            }
            if (!$id_groups || (is_array($id_groups) && count($id_groups) <= 0)) {
                return [];
            }
        } else {
            if ($is_subadmin) {
        ...
        switch ($query_type) {
            ...
            case 'standard':
				// ABR
                $query = 'SELECT DISTINCT u.idst, u.userid, u.lastname, u.firstname, u.email, u.register_date, u.lastenter, u.valid, u.privacy_policy '
                    . ' FROM %adm_user as u ';
        ...
        if (count($users_rows) > 0) {
            ...
            if ($descendants) {
                ...
                foreach ($users_rows as $idst => $value) {
                    $users_rows[$idst]['is_descendant'] = false;
                }
            }
            ...
            while (list($idst, $value, $type) = $this->db->fetch_row($res_others)) {
                switch ($type) {
                    case 'language':
                        $users_rows[$idst]['language'] = $value;
                        break;
                    case 'level':
                        $users_rows[$idst]['level'] = $levels_flip[$value];
                        break;
                }
            }
        }
    ...
    public function getTotalUsers($idOrg, $descendants = false, $filter = false, $usersFilter = false, $learning_filter = 'none')
    {
        ...
        if ($is_subadmin && $is_dynfiltered) {
            ...
            if (count($t_arr) <= 0) {
                return 0;
            } //no users can be extracted at this point: no more querying is necessary
    ...
    public function suspendUsers($users)
    {
		...
		$gap_model 	= new GapAlms();
		$assn_model = new CourseassnAlms();
		$retVal 	= false;
		...
		//Annullo eventuali gap e assegnazioni
		if($retVal) {
			//$gap_model->cancelGapByUser($users);
			//$assn_model->cancelAssnByUser($users);
		}
                ...
                if (is_array($folders) && !empty($folders)) {
                    foreach ($folders as $folder) {
                        if ((int) $folder > 0) {
                            ...
                            $enrollrules->newRules('_NEW_USER', [$user_idst], $lang_code, $folder);
                        }
                    }
                }
            ...
            if (count($admtree) > 0) {
                ...
                if (count($orgtree) > 0) { //this should be always true ...
                    ...
                    while (list($idorg, $left, $right) = $this->db->fetch_row($res)) {
                        $this->orgCache[$idorg] = [$left, $right];
                    }
                }
            }
        }
    ...
    public function getAllFolders($use_as_key = false, $descendantof = false)
    {
        ...
        while ($folder = $this->db->fetch_obj($result)) {
            if ($use_as_key != 'both') {
                $folders[trim(strtolower($folder->$use_as_key))] = $folder;
            } else {
                $folders[trim(strtolower($folder->translation))] = $folder;
                if ($folder->code != '') {
                    $folders[trim(strtolower($folder->code))] = $folder;
                }
            }
        }
     ...
     */
    public function addFolder($id_parent, $langs, $code = '', $id_template = 0)
    {
        $output = false;
        ...
        if (is_array($langs)) {
            ...
            $query = 'INSERT into %adm_org_chart_tree (idOrg, idParent, path, lev, iLeft, iRight, code, associated_template, date_created) VALUES '
                . "(NULL, '" . (int) $id_parent . "', '" . $path . "', '" . (int) $level . "', " . (int) $new_limits['iLeft'] . ', ' . ((int) $new_limits['iRight'] + 1) . ", '" . $code . "', '" . $template_name . "', NOW())";
            $res = $this->db->query($query);
    ...
    public function deleteFolder($idOrg, $onlyLeaf = false)
    {
        ...
        $res = $this->db->query($query);
        $nodes = [];
        while (list($node) = $this->db->fetch_row($res)) {
            $nodes[] = $node;
        }
...
*/
    ...
    public function randomPassword($idst)
    {
        ...
        if ($this->changePassword($idst, $new_password)) {
            ...
            if (!empty($new_password)) {
                $msg_composer->setBodyLangText('email', '_PASSWORD_CHANGED', ['[password]' => $new_password]);
    ...
    public function getWaitingUsersList($pagination = [], $filter = false)
    {
        if (!is_array($pagination)) {
            $pagination = [];
        }
        ...
        if (isset($pagination['sort'])) {
            switch ($pagination['sort']) {
                case 'firstname':
                    $sort = 'w.lastname ' . $dir . ', w.firstname';
                    break;
                case 'lastname':
                    $sort = 'w.firstname ' . $dir . ', w.lastname';
                    break;
                case 'email':
                    $sort = 'w.email';
                    break;
    ...
    public function getDeletedUsersList($pagination = [], $filter = false)
    {
        if (!is_array($pagination)) {
            $pagination = [];
        }
        ...
        if (isset($pagination['sort'])) {
            switch ($pagination['sort']) {
                case 'userid':
                    ...
                    break;
                case 'firstname':
                    $sort = 'd.lastname ' . $dir . ', d.firstname';
                    break;
                case 'lastname':
                    $sort = 'd.firstname ' . $dir . ', d.lastname';
                    break;
                case 'email':
                    $sort = 'd.email';
                    break;
                case 'deletion_date':
                    $sort = 'd.deletion_date';
                    break;
                case 'deleted_by':
                    $sort = 'deleted_by';
                    break;
    ...
    public function updateMultipleUsers($users, $info)
    {
        ...
        if (property_exists($info, 'firstname')) {
            $user_conditions[] = " firstname='" . $info->firstname . "' ";
        }
        if (property_exists($info, 'lastname')) {
            $user_conditions[] = " lastname='" . $info->lastname . "' ";
        }
        if (property_exists($info, 'email')) {
            $user_conditions[] = " email='" . $info->email . "' ";
        }
        if (property_exists($info, 'password')) {
            $user_conditions[] = "pass='" . $this->aclManager->encrypt($info->password) . "' ";
        }
        if (property_exists($info, 'force_change')) {
            $user_conditions[] = 'force_change=' . ($info->force_change ? '1' : '0');
        }
