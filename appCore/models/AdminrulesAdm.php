...
class AdminrulesAdm extends Model
{
                ...
                if (!empty($perm)) {
                    foreach ($perm as $perm_name => $img) {
                        if (array_search($perm_name, array_keys($total_perm)) == false) {
                            ...
                            if ($collapse === 'true') {
                                $th = [''];
                            }//array(Lang::t($default_name, 'menu'));
                        }
	...
	public function getGroupByUser($id_user) {
		//>> ABR: Restituisce il gruppo o i gruppi a cui appartiene l'utente amministratore passato in argomento
	
		$res = array();
		
		// Preparo query
		$query =	"SELECT g.idst, g.groupid"
					." FROM %adm_group g"
					."  JOIN %adm_group_members as m ON g.idst = m.idst"
					." WHERE groupid LIKE '".$this->rules_path."%' AND m.idstMember = ". (int)$id_user
					." ORDER BY groupid ";
		...
		while( list($idst, $groupid) = sql_fetch_row($result) ) {
			...
			$res[$idst] = array('idst' => $idst, 'groupid' => $name );
		}
