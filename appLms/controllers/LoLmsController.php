...
class LoLmsController extends LmsController
{
    ...
    private function getFolders($idCourse, $idFolder = false)
    {
        ...
        foreach ($results as &$item) {
            if ($item['is_folder']) {
                $item['folderStatus'] = $this->model->getFolderStatusById($item['id']);
            }
        }
        unset($item);
    ...
    public function show()
    {
        ...
        $folders = $this->getFolders($this->idCourse, false);
        $rootStatus = $this->model->getRootFolderStatus();
        $statusMap = ['0' => $rootStatus];
        foreach ($folders as $folder) {
            if (!empty($folder['is_folder'])) {
                $statusMap[(string) $folder['id']] = isset($folder['folderStatus']) ? (int) $folder['folderStatus'] : 0;
            }
        }
        ...
        $this->render('show', [
            'data' => [
                ...
                'type' => 'organization',
                'folderStatusJson' => json_encode($statusMap),
            ],
        ]);
    }
    ...
    {
        $folders = $this->getFolders($this->idCourse, false);
        $statusMap = ['0' => $this->model->getRootFolderStatus()];
        foreach ($folders as $folder) {
            if (!empty($folder['is_folder'])) {
                $statusMap[(string) $folder['id']] = isset($folder['folderStatus']) ? (int) $folder['folderStatus'] : 0;
            }
