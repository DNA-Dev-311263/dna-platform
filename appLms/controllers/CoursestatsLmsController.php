...
class CoursestatsLmsController extends LmsController
{
    ...
    public function gettabledataTask()
    {
        ...
        if (is_array($list)) {
            ...
            foreach ($list as $record) {
                ...
                $row = [
                    // 'id' => (int)$record->idst,
                    'restrack_id' => (int)$record->idst, 	 //ABR, id utente da azzerare
    ...
    /**
     * Reset i LO fruiti da un utente in uno specifico corso (chiamata via ajax)
     * (ABR)
     */
 	public function resetall() {
		...
		// Errore predefinito
		$output = array('success' => false, 'message' => $this->_getErrorMessage("reset error"));
		...
		// Reset
		$subs_model = new SubscriptionAlms($id_course);
		$res = $subs_model->resetCourseUserTrackInfo($id_user);

		// Out
		if ($res)	
			$output = array('success' => true, 'user' => $id_user);
