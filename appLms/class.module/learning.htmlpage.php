...
class Learning_Htmlpage extends Learning_Object
{
    ...
    public function play($id, $id_param, $back_url)
    {
        ...
        // NOTE: Track only if $idReference
        if ($idReference !== false) {
            
            //ABR Trovo se è valorizzata la proprietà videoComplete
			$query = "SELECT videoComplete FROM %lms_organization WHERE idOrg = " . $idReference;
			list($videoComplete) = sql_fetch_row(sql_query($query));
       
            // Trovo ID tracciamento
            ...
            if (! $exist) {
				//ABR: Se non esiste ancora un tracking
				// Lo stato è 'completed' all'apertura solo se non c'è gestione avanzamento video
				$status = (! $videoComplete) ? 'completed' : 'ab-initio';
				$ti = new Track_Htmlpage(false);
                $ti->createTrack($idReference, $idTrack, getLogUserId(), date('Y-m-d H:i:s'), $status, 'htmlpage'); //ABR
			
			} elseif (! $videoComplete) {
				//ABR: Se esiste un tracking
				// aggiorno il tracking in apertura solo se non c'è video
		...
		//ABR  Aggiungo librerie js
		Util::get_js(FormaLms\lib\Get::rel_path('lms') . '/modules/organization/organization-htmlpage.js', true, true);
		//$GLOBALS['page']->add('<script type="text/javascript" src="' . $GLOBALS['where_lms_relative'] . '/modules/organization/organization-htmlpage.js"></script>', 'page_head');
        ...
        $GLOBALS['page']->add('<script type="text/javascript">', 'content'); //ABR
        $GLOBALS['page']->add('Htmlpage.init({idItem:'.$idReference.', videoComplete:'.$videoComplete.'})', 'content'); //ABR
        $GLOBALS['page']->add('</script>', 'content'); //ABR
     }
    ...
    //ABR consento opzione oggetto completato al progress del video
    public function canBeVideoComplete()
    {
        return true;
    }
