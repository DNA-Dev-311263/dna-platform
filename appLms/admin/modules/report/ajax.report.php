...
switch ($op) {
	...
	case 'profile_admins': {
		// ABR: restituisce tutti gli utenti di uno o più gruppi
		
		require_once(_lms_.'/admin/modules/report/class.report.php');
		
		$json = new Services_JSON();
		$report = new Report(false);
		$p_idst = FormaLms\lib\Get::req('idst', DOTY_MIXED, false);
		$res = array('success' => false);

		// Recupero array id profili
		if ($p_idst)
			$p_idst = explode(",", $p_idst);
		
		// Recupero utenti amministratori
		$all_members = $report->getProfileAdmins($p_idst);

		//Preparo output
		$res = array('success' => true, 'count' => 0);
			
		if ($all_members) {
			$res['count'] = count($all_members);
			$res['fields'] = array(	['key'=>'userid', 'label'=>Lang::t('_USERNAME', 'standard')], 
									['key'=>'lastname', 'label'=>Lang::t('_LASTNAME', 'standard')], 
									['key'=>'firstname', 'label'=>Lang::t('_FIRSTNAME', 'standard')] );
				
			$res['data'] = $all_members;
		}

			
		//Out
        aout($json->encode($res));
