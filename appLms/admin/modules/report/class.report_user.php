...
class Report_User extends Report
{
    ...
    public function get_courses_filter()
    {
        ...
        $box->body .= Form::getOpenFieldset(
            ...
            . Form::getCheckBox($lang->def('_TIME_USED'), 'perc_lo', 'cols[]', '_TH_TIME_USED_LO', $this->is_showed('_TH_TIME_USED_LO')) //ABR
        ...
        $classrooms_editions_info)
    {
        ...
        // Luca & ABR
         if (in_array('_TH_TIME_USED_LO', $cols)) { 
            $th2[] = $lang->def('_TIME_USED'); $colspanLO++; 
        } 
        ...
        //ABR (se non ci sono colonne per questa sezione non mostro header1)
        if ($colspan2) {
        ...
        while ($sql_row = sql_fetch_array($re_course_user)) {
            ...
            if ($condition) {
                ...
                if (in_array('_TH_ESTIMATED_TIME', $cols)) {
 				...
 				// ABR
				if (in_array('_TH_TIME_USED_LO', $cols)){ 
                    $time_used = $this->getTimeUsedLO($id_user, $id_course);
	...
	function getTimeUsedLO($id_user, $id_course) {
		//>> ABR: Restituisce il tempo di fruzione sugli scorm di un corso
		
		$query	= "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(th.session_time))) AS total_time "
				. " FROM %lms_organization AS org "
				. " JOIN %lms_scorm_tracking AS tk ON ( org.objectType = 'scormorg' AND org.idOrg = tk.idReference )"
                . " LEFT JOIN %lms_scorm_tracking_history AS th ON th.idscorm_tracking = tk.idscorm_tracking " 
				. " WHERE tk.idUser = " .(int)$id_user. " AND org.idCourse = " .(int)$id_course
                . " GROUP BY tk.idUser, org.idCourse";
    ...
    public function _get_LO_query($type = 'html', $report_data = null, $other = '')
    {
        ...
        //retrievescorm score (FIX ABR)
        $query = 'SELECT t1.idOrg, t2.idUser, t1.idCourse, t2.score_raw, t2.score_min, t2.score_max, '
            . " SEC_TO_TIME(SUM(
					CASE 
						WHEN th.session_time LIKE 'PT%'
							THEN CAST(SUBSTRING(th.session_time, 3, LENGTH(th.session_time)-3) AS UNSIGNED)
						ELSE TIME_TO_SEC(th.session_time)
					END
				)) AS total_time "
            . ' FROM %lms_organization AS t1 '
            ...
            . (!$all_users ? ' AND t2.idUser IN (' . implode(',', $users) . ') ' : '')
            . (count($tempmilestones) > 0 ? ' AND t1.milestone IN (' . implode(',', $tempmilestones) . ') ' : '')
            . ' GROUP BY t1.idOrg, t2.idUser, t1.idCourse, t2.score_raw, t2.score_min, t2.score_max;';
        ...
        switch ($order_by) {
            ...
            case 'object_type':
                ...
                break;
            case 'first_attempt':
                $query_order_by = 't2.firstAttempt '. $_dir; //ABR
                break;
            case 'last_attempt':
                $query_order_by = 't2.dateAttempt '. $_dir; //ABR
                break;
        }
