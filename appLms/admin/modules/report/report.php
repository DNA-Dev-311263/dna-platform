...
function get_report_table($url = '')
{
    ...
    switch ($level) {
        ...
        default:
			// Mod. ABR
			$id_user = $current_user[ACL_INFO_IDST];
			$author_filter = $reportAdminFilter['author'];
			$report_allowed = getReportAllowedByList( $id_user );
			
    		if ($author_filter > 0) {
				$qconds[] = " ( t1.author = ".$author_filter." AND (t1.is_public = 1 OR t1.is_public = 2 AND t1.id_filter IN ('". implode("','", $report_allowed). "')) )";
			} else {
				 $qconds[] = " ( t1.author = ".$id_user." OR t1.is_public = 1 OR (t1.is_public = 2 AND t1.id_filter IN ('". implode("','", $report_allowed). "')) ) ";
			}
    ...
    $copy_icon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#666" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>';

    if ($level == ADMIN_GROUP_GODADMIN || $can_mod) {
        //insert the "copy" column right before the export-XLS column (3rd icon from left)
        $xls_pos = count($col_content) - 1;
        array_splice($col_content, $xls_pos, 0, [$copy_icon]);
        array_splice($col_type, $xls_pos, 0, ['image']);
    }
    ...
    if ($res = sql_query($query)) {
        foreach ($res as $row) {
            ...
            $copy_link =
                '<a href="' . $url . '&amp;action=copy&amp;idrep=' . $id . '" ' .
                ' title="Copia : ' . ($row['author'] == 0 ? $lang->def($row['filter_name']) : $row['filter_name']) . '">' .
                '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#666" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>' .
                '</a>';
            ...
            //ABR   
			$p_img[0] =  '<image title="'.$lang->def('_PRIVATE').'" src="'.getPathImage('lms').'standard/unpublish.png" />';
			$p_img[1] =  '<image title="'.$lang->def('_PUBLIC').'" src="'.getPathImage('lms').'standard/publish.png" />';
			$p_img[2] =  '<image title="'.$lang->def('_LIST').'" src="'.getPathImage('lms').'standard/publish_list.png" />';
			
			$public = $p_img[ $row['is_public'] ];
            ...
            if ($level == ADMIN_GROUP_GODADMIN || $can_mod) {
                //insert the "copy" link right before the export-XLS link (3rd icon from left)
                $xls_pos = count($tb_content) - 1;
                array_splice($tb_content, $xls_pos, 0, [_REP_KEY_COPY => $copy_link]);
            }
...
function report_rows_filter()
{
    ...
    $is_mod_pub = (FormaLms\lib\Get::req('is_public', DOTY_MIXED, false) !== false && checkPerm('mod_public', true)); // ABR
    ...
    if (FormaLms\lib\Get::req('set_category', DOTY_INT, 0) == 1) {
        ...
        /* ABR */
		if ($is_mod_pub) {	
			$reportTempData['is_public'] = \FormaLms\lib\Get::req('is_public', DOTY_INT);
			$report_recipients = FormaLms\lib\Get::req('report_recipients', DOTY_STRING, "");
			$reportTempData['report_recipients'] = explode(",", $report_recipients);
		}
		
		//ABR: Salvo variabili sessione
...
function report_save_filter()
{
    ...
    $is_public = ( isset($reportTempData['is_public']) ? $reportTempData['is_public'] : false ); //ABR
    ...
    if ($session->get('report_update') !== null || $idrep) {
		$save_ok = report_update($idrep, $report_name, $reportTempData, $is_public); //ABR
        if ($show) {
        ...
        $save_ok = report_save($report_id, $report_name, $reportTempData, $is_public); //ABR
        if ($show) {
...
function report_open_filter()
{
    ...
    switch ($action) {
        ...
        case 'copy':
            //duplicate the filter ("Copia di ..."), than reload page
            if ($new_filter_id = report_copy_filter($filter_id)) {
                $success = '&fcopy=1&idrep=' . $new_filter_id;
            } else {
                $success = '&fcopy=0&idrep=' . $filter_id;
            }
            Util::jump_to($url . $success);
...
function ins_mod_public($idrep) {
	//ABR
	
	// Controllo i permessi
	checkPerm('mod');
	if ($idrep) canAccess($idrep);
	$can_mod_pub = checkPerm('mod_public', true);
	...
	$is_godadmin = ($level == ADMIN_GROUP_GODADMIN);
	$obj_report = new Report($idrep);
	
	$img_view = '<img src="'.getPathImage().'standard/view.png" alt="'.$lang->def('_VIEW').'" title="'.$lang->def('_VIEW').'" />';

	$output = "";
	
	
	// Dropdown
	if ($is_godadmin || ($can_mod_pub && $idrep)) {
		// Presento la casella di selezione solo se è godadmin o amministratore in modifica
		
		$is_public = 0;
		
		//Recupero le info del report se si è in modifica
		if ($idrep) {
			$report = getReportById($idrep);
			$is_public = $report['is_public'];
		}

		$options[0] = $lang-> def('_PRIVATE');
		$options[1] = $lang-> def('_PUBLIC');
		$options[2] = ($is_godadmin ? $lang->def('_LIST') : $lang->def('_LIST_LOCKED'));
	
		$span_preview = "<span id='adm_list_preview' style='margin-left:15px; visibility:hidden; cursor:pointer;'>".$img_view."</span>";
		
		$output .= Form::getBreakRow();
		$output .= Form::getDropdown($lang->def('_TAB_REP_PUBLIC'), 'is_public', 'is_public', $options,  $is_public, $span_preview);
	
		//Casella per id profili selezionati
		$output .= Form::getHidden("report_recipients", "report_recipients", implode(",", $reportTempData['report_recipients']));
		$output .= Form::getBreakRow();
	...
	if ($is_godadmin) {
		...
		// Recupero i profili / gruppi di amministratore
		$profiles = $obj_report->getAdminProfile();
				
		// Preparo array dati
		foreach($profiles as $k => $row) {
			
			$data[$k]['selected'] =  Form::getCheckbox(false, 'sel_'.$k , 'selgroup', $k, $row['selected']);
			$data[$k]['name'] = $row['name'];
			$data[$k]['adm_count'] = $row['adm_count'];
			$data[$k]['adm_view'] = "<span style='cursor:pointer;' class='adm_view_link' data-name='".$row['name']."' data-idst='".$k."'>".$img_view."</span>";	
		}
				
		// Preparo altri parametri widget
		$styles = array('selected' => 'image', 'adm_view' => 'image');
		$header = array('selected' => '<a href="javascript:PublicModify.selectAll();">'.Lang::t('_SELECT', 'standard').'</a>',
						'name' =>Lang::t('_NAME', 'standard'),
						'adm_count' => Lang::t('_ADMINISTRATORS'),
						'adm_view' => $img );
						

		$params = array(
			'caption'=> $lang->def('_RECIPIENTS_GROUP'),
			'id' => 'adminprofile_table',
			'styles' => $styles,
			'data' => $data,
			'header' => $header,
		);
		...
		// Recupero tabella 
		$output .= "<div id='box_adminprofile_table' style='display: none;'>";
		$output .= 		Util::widget('table', $params, true);
		$output .= "</div>";
	}
	
	if ($output) {
		// Aggiungo librerie JS per l'output
		Util::get_js( FormaLms\lib\Get::rel_path('base').'/lib/js_utils.js', true, true );
		Util::get_js( FormaLms\lib\Get::rel_path('lms').'/admin/modules/report/public_modify.js', true, true );
		
		// Aggiungo script
		$output .= PHP_EOL ."
			<script type='text/javascript'>
				PublicModify.init({
					startLink: 'index.php?modname=report&op=reportlist',
					ajaxPath: '".FormaLms\lib\Get::rel_path('lms')."/ajax.adm_server.php?mn=report&plf=lms',
					allPerm: ". (int)$is_godadmin. ",
					langs:	{ _ADMIN_MANAGMENT_CAPTION: '". Lang::t('_ADMIN_MANAGMENT_CAPTION', 'adminrules'). "',
							  _NO_CONTENT: '". Lang::t('_NO_CONTENT', 'standard'). "',
							  _PREVIEW: '". Lang::t('_PREVIEW', 'standard'). "'
							}
				});	
			</script>";
			
		// Aggiugno div per popup
		$output .= "<div id='resizablepanel'></div>";
	}
