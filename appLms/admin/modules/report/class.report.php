...
class Report
{
	...
	public function getProfileAdmins($p_idst = false){
		//>> ABR: restituisce tutti gli utenti di uno o più gruppi (usata via ajax)
		...
		$model = new AdminrulesAdm();
	
		
		// Recupero id gruppi in base alla chiamata e al ruolo
		if ( $level==ADMIN_GROUP_GODADMIN && $p_idst )
			$arr_idst = ( !is_array($p_idst) ? array($p_idst) : $p_idst );
			
		elseif ( $level==ADMIN_GROUP_ADMIN || $level==ADMIN_GROUP_USER )
			$arr_idst =  ( isset($_SESSION['report_tempdata']['report_recipients']) ?  $_SESSION['report_tempdata']['report_recipients'] : array() );

			
		// Recupero tutti i membri per ogni gruppo
		foreach($arr_idst as $idst) {
			
			$members = $model->loadAdmin( (int)$idst, false, false, "lastname", 'ASC');
			
			foreach ($members as $row) {
				$all_members[] = array(	'userid' => $row['userid'],
	...
	function getAdminProfile() {
		//>> ABR: restituisce i profili amministratore con l'indicazione di quelli già selezionati
		...
		// Recupero i profili già selezionati per il report
		if ( isset($reportTempData['report_recipients']) )
			$report_recipients =  $reportTempData['report_recipients'];
		
		// Carico i profili
		$result = $model->loadGroup(false, false, false, '');
		...
		foreach ($result as $row) {
			...
			// Recupero gli amministratori
			$members = $model->loadAdmin($idst, false, false, "lastname", 'ASC');
		
			// Conteggio amminsitratori per profilo
			$adm_count = count($members);
			...
			$res[$idst] = array (
				...
				'name' => $row['groupid'],
				'adm_count' => $adm_count,
				'selected' => in_array($idst, $report_recipients)
			);
...
// addCss('style_filterbox'); fix ABR
