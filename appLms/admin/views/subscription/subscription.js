...
E.onAvailable("multimod_date_complete", function() {
		...
		if (c) c.disabled = true;
					...
					callback: function(o) { try {
                        if (o.success && o.message) {
                            WriteDialogMessage(this, o.message); //ABR dic-21
                            oDt.refresh();
			...
			//multi reset, ABR
			var multireset_links = YAHOO.util.Dom.getElementsByClassName('ico-wt-sprite subs_reset');

			YAHOO.util.Event.addListener(multireset_links, "click", function(e) {
				var body = '', count_sel = oDtS.num_selected;

				if (count_sel > 0) {
					body += '<form method="POST" id="subscribe_table_multireset_dialog_form" action="'+this.href+'">'
						+'<p>'+S.oLangs.get('_RESET')+': '+count_sel+' '+S.oLangs.get('_USERS')+'</p>'
						+'<input type="hidden" name="users" value="'+oDtS.toString()+'" />'
						+'</form>';
				} else {
					body += '<p>'+S.oLangs.get('_EMPTY_SELECTION')+'</p>';
				...
				var oDialog = CreateDialog("subscribe_table_multiresetDialog", {
					width: "500px",
					modal: true,
					close: true,
					visible: false,
					...
					callback: function(o) {
						if (o.reset) {
							//var i;
							//for (i=0; i<o.reset.length; i++)
							//	oDtS.remsel(o.reset[i]);
						}
						this.destroy();
						oDt.refresh();
					}
