<?php 
	...
	echo getTitleArea(Lang::t('_MANAGE_COURSEASSN', 'menu'));
...
?>
<div class="std_block">
	<div class="page_data_filter" id="courseassn_filter" style="display: block;">
		<?php
				
			// Preparo combo per filtri
			$curr_year = date('Y');
			$min_year = $model->getAssnYearMin();
			...
			if($is_godadmin) {
				echo Form::getLineDropdown('filt_param', 'lbl_f_par', Lang::t('_ORGANIZATION', 'courseassn'), 'sel_f_par', 'filter_org', 'filter_org', $model->getOrgForDropdown(), $id_org, '','','');
			} else {
...
/*-------------------------- COLONNE DEL DATATABLE --------------------------*/
	...
	if ($permissions['mod']) {
		$columns[] = array('key' => 'del', 'label' => Formalms\lib\Get::img('standard/delete.png', Lang::t('_DEL', 'subscribe')), 'formatter'=>'doceboDelete', 'className' => 'img-cell');
	}
...
/*-------------------------- PREPARAZIONE WIDGET --------------------------*/
	...
	$_params = array(
		'id'			=> 'courseassn_table',
		'ajaxUrl'		=> 'ajax.adm_server.php?r='.$base_link_courseassn.'/getCourseassnList&id_fncrole_ref=&',
		...
		'stdSelection' => true,
		'selectAllAdditionalFilter' => 'Courseassn.selectAllFilter()',
		'delDisplayField' => 'description',
		'generateRequest' => 'Courseassn.requestBuilder',
		'editorSaveEvent' => 'Courseassn.editorSaveEvent',
		'events' => array(
			'initEvent' => 'Courseassn.initEvent',
			'beforeRenderEvent' => 'Courseassn.beforeRenderEvent',
			'postRenderEvent' => 'Courseassn.postRenderEvent'
		)
	);
	...
	echo Form::openForm('csv_form', "index.php?r=".$base_link_courseassn."/csvexport");
	...
	$onclick_js = "location.href='index.php?r=".$base_link_courseassn."/insertAssnWizard&";
	...
	if ($permissions['mod']) 	
		$rel_action .= '<a class="ico-wt-sprite subs_del" href="ajax.adm_server.php?r='.$base_link_courseassn.'/multidel"><span>'.Lang::t('_DEL_SELECTED', 'subscribe').'</span></a>';
...
Courseassn.init({
	...
	baseLink: "<?php echo $base_link_courseassn; ?>",
	...
	perms: {
		mod_assn: <?php echo $permissions['mod'] ? 'true' : 'false'; ?>,
		view_assn: <?php echo $permissions['view'] ? 'true' : 'false'; ?>
	},
	langs: {
		_ASSIGNMENTS: "<?php echo Lang::t('_ASSIGNMENTS', 'standard'); ?>",
		...
		_EXPORT_ASSN_ACTIVE:  "<?php echo Lang::t('_EXPORT_ASSN_ACTIVE', 'courseassn'); ?>",
		_UPDATE_ASSN: "<?php echo Lang::t('_UPDATE_ASSIGNMENT', 'courseassn'); ?>",
		_CANCEL_ASSN_ACTIVE: "<?php echo Lang::t('_CANCEL_ASSN_ACTIVE', 'courseassn'); ?>",
