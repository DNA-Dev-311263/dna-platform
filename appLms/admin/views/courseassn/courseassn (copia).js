	...
	init: function(oConfig) {
		...
		YAHOO.util.Event.onDOMReady(function(e) {
            ...
            var updateManager = function() {
                ...
                //window.location = "index.php?r="+base_link+"/importAssnWizard&";
			...
			*/
			...
			//multi delete
			var getDialogIcon  = function(icon){return '<span class="yui-icon '+icon+'">&nbsp;</span>'};
			...
			YAHOO.util.Event.addListener(multidel_links, "click", function(e) {
				...
				if (count_sel > 0) {
					body += getDialogIcon('warnicon')
						+'<form method="POST" id="courseassn_table_multidel_dialog_form" action="'+this.href+'">'
						+'<p>'+A.oLangs.get('_DEL')+': '+count_sel+' '+A.oLangs.get('_ASSIGNMENTS')+'</p>'
						+'<input type="hidden" name="assignments" value="'+oDtS.toString()+'" />'
						+'</form>';
				} else {
					body += getDialogIcon('infoicon') +'<p>'+A.oLangs.get('_EMPTY_SELECTION')+'</p>';
				}

				var oDialog = CreateDialog("courseassn_table_multidelDialog", {
					width: "500px",
					modal: true,
					close: true,
					visible: false,
					...
					header: A.oLangs.get('_AREYOUSURE'),
					body: body,
					callback: function(o) {
						if (o.deleted) {
							var i;
