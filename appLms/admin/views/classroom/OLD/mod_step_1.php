...
echo Form::openForm('add_date', 'index.php?r=' . $base_link_classroom . '/updateClassroom&amp;id_course=' . $model->getIdCourse() . '&amp;id_date=' . $model->getIdDate())
        ...
        . Form::getTextfield(Lang::t('_INTERNAL_NOTE', 'classroom'), 'internal_note', 'internal_note', 255, (isset($date_info['internal_note']) ? stripslashes($date_info['internal_note']) : stripslashes($_POST['internal_note'])))
...
YAHOO.util.Event.onDOMReady(function()
{
	var calendar = new YAHOO.widget.CalendarGroup("calendar", "calendar_container", {
		...
		MULTI_SELECT: true
		<?php if ($start_mounth != '') {
    echo ', pagedate: "' . $start_mounth . '"';
} ?>
	});

	<?php if ($date_string != '') {
    echo 'calendar.cfg.setProperty("selected", "' . $date_string . '", false);';
} ?>
