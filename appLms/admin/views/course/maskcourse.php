...
<div class="std_block">
    ...
    if ($course['course_type'] == 'classroom' && $has_editions_or_classrooms) {
        ...
        $htmlData .= (Form::getLineBox(Lang::t('_COURSE_TYPE', 'course'), $course_type['classroom'])
            . Form::getHidden('course_type', 'course_type', 'classroom'));
    } elseif ($course['course_edition'] > 0 && $has_editions_or_classrooms) {
        //this is a classroom course with editions
        $htmlData .= (Form::getLineBox(Lang::t('_COURSE_TYPE', 'course'), $course_type['edition'])
            . Form::getHidden('course_type', 'course_type', 'edition'));
    } else {
        //$htmlData .= (Form::getDropdown(Lang::t('_COURSE_TYPE', 'course'), 'course_type', 'course_type', $course_type, $id_course === false ? 'elearning' : $course['course_type']));
        //ABR: cambio il tipo corso di default
        $htmlData .= (Form::getDropdown(Lang::t('_COURSE_TYPE', 'course'), 'course_type', 'course_type', $course_type, $id_course === false ? 'classroom' : $course['course_type'] ));
    }
    $htmlData .= (Form::getDropdown(Lang::t('_STATUS', 'course'), 'course_status', 'course_status', $status, $course['status'])
    	. Form::getCheckbox(Lang::t('_VIRTUAL', 'course'), 'course_virtual', 'course_virtual', '1', $course['course_virtual'] == 1)		//ABR
        ...
        . Form::getSimpleTextarea(Lang::t('_COURSE_BOX_DESCRIPTION', 'course'), 'course_box_descr', 'course_box_descr', $course['box_description'], false, false, false, 5, 22, '', 500) //ABR Mod. capacità capacità campo
        ...
        . Form::getDropdown(Lang::t('_COURSE_LANG_METHOD', 'course'), 'course_lang', 'course_lang', $array_lang_label, array_search($course['lang_code'], $array_lang))
    ...
    $htmlData .= (Form::getCloseFieldset());

    $htmlData .= (Form::openCollasableFieldset(Lang::t('_COURSE_SUBSCRIPTION', 'course'))
        //-----------------------COURSE SUBSCRIPTION ---------------------
        ...
        . Form::getRadio(Lang::t('_COURSE_S_BY_ASSIGNMENT', 'course'), 'course_subs_by_assignment', 'course_subs', '9', $course['subscribe_method'] == 9) //ABR
        . Form::getRadio(Lang::t('_COURSE_S_BY_GAP', 'course'), 'course_subs_by_gap', 'course_subs', '8', $course['subscribe_method'] == 8) //ABR - sospeso
        . Form::getCloseCombo()
        ...
        . Form::getTextfield(Lang::t('_COURSE_PRIZE', 'course'), 'course_prize', 'course_prize', '11', $course['prize'] != '' ? number_format((float)(strpos($course['prize'], ',') !== false ? str_replace(',', '.', str_replace('.', '', $course['prize'])) : $course['prize']), 2, ',', '.') : '')
        ...
        . Form::getCheckbox('Includi nella pagina catalogo pubblico', 'course_in_home_catalogue', 'course_in_home_catalogue', '1', $course['in_home_catalogue'] == 1)
    ...
    foreach ($levels as $level => $level_name) {
        $htmlData .= (Form::getCheckbox($level_name, 'course_show_level_' . $level, 'course_show_level[' . $level . ']', $level, $course['level_show_user'] & (1 << $level)));
    }
    $htmlData .= (Form::getCloseCombo()
        ...
        . Form::getTextfield(Lang::t('_NEW_CONTENT_INDICATOR_DAYS', 'course'), 'new_content_indicator_days', 'new_content_indicator_days', '11', $course['new_content_indicator_days']) //ABR
    ...
    if ($fman->getNumberFieldbyArea() > 0) {
        ...
        $htmlData .= (Form::openCollasableFieldset(Lang::t('_CUSTOM_OPTIONS', 'course'))
            . $fields_mask
            . Form::getCloseFieldset());
    }
    ...
    $eventResult = Events::trigger('lms.course.mask.showing', ['idCourse' => $id_course, 'courseData' => $course, 'htmlData' => $htmlData]);
    $htmlData = $eventResult['htmlData'];
...
<script type="text/javascript">
	...
	var p = D.get("course_prize");
	
	setCourseTypeSubs();
	p.value = formatNumber(parseFloatString(p.value));
    ...
    E.onDOMReady(function () {
		...
		E.addListener("course_type", "change", setCourseTypeSubs);
		
		E.addListener("course_prize", "change", function(e) {
			p.value = formatNumber(parseFloatString(p.value));
		});
		E.addListener("maskcourse_form", "submit", function(e) {
			p.value = parseFloatString(p.value);
		});
    });
    
 /** ABR **/
	 function setCourseTypeSubs(){
		 
		var t = D.get("course_type");
		var	a = D.get("course_subs_by_assignment");
		var	g = D.get("course_subs_by_gap");
		var v = D.get("course_virtual");

		if(t !== null){
			var val = t.value;
			
			if(val == 'classroom' || val == 'edition'){
				a.disabled = false;
				g.checked = false; g.disabled = true;
				
			}else if(val == 'assessment' ){
				a.checked = false; a.disabled = true;
				g.disabled = false;	
			...
			if(val == 'classroom'){
				...
				v.checked = false; v.disabled = true;
			}
		}
	}
	...
	function parseFloatString(stringValue) {
		...
		var intStr = '';
		var decStr = '';
		var numStr = stringValue.trim();
		var result = null;
		
		if  (/[,\.]\d+$/.test(numStr)) {
			intStr = numStr.replace(/[,\.]\d+$/, '');
			decStr = numStr.replace(intStr, '').replace(/[,\.]/, '');

			numStr = intStr.replace(/[,\.]/, '') + '.' + decStr;
		} 
