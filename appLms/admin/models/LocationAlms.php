...
class LocationAlms extends Model
{
    ...
    {
		//Mod. ABR
        $query = 'SELECT location_id AS id_location, location, location_virtual '
            . ' FROM  %lms_class_location '
            . ' WHERE 1 ';
        if ($filter != '') {
            $query .= " AND location LIKE '%" . $filter . "%'";
        }
        if ($sort && $dir) {
	...
	public function insertLocation($location, $location_virtual) 
	{
		//Mod. ABR
		$query = "INSERT INTO %lms_class_location "
				. " (location, location_virtual) VALUES ("
				. " '" . $location . "', " . (int)$location_virtual
				. ")";
    ...
    public function updateLocation($location_id, $location_new, $location_virtual)
    {
		//Mod. ABR
		$query = " update %lms_class_location ";
		$query .= " set location = '" . $location_new . "', location_virtual = " . (int)$location_virtual;
    ...
    public function getClassroomDates2Date($id_classroom, $date, $startIndex, $results, $sort = false, $dir = false, $filter = false)
    {
        ...
        if ($position == 1) {
            $date_in = substr($date, 2, 5) . '-0' . substr($date, 0, 1);
        } else {
            $date_in = substr($date, 3, 5) . '-' . substr($date, 0, 2);
        }

        if ($position == 1) {
            $date1_month = substr($date, 0, 1);
        } else {
            $date1_month = substr($date, 0, 2);
        }
        if ($position == 1) {
            $date1_year = substr($date, 2, 5);
        } else {
            $date1_year = substr($date, 3, 5);
        }

        if (($date1_month > 0) && ($date1_month < 10)) {
        ...
        if ($position == 1) {
            $date2_month = substr($date, 0, 1);
        } else {
            $date2_month = substr($date, 0, 2);
        }
        ++$date2_month;
        if ($position == 1) {
            $date2_year = substr($date, 2, 5);
        } else {
            $date2_year = substr($date, 3, 5);
        }
        if ($date2_month == 13) {
            $date2_month = 1;
            ++$date2_year;
        }
        if (($date2_month > 0) && ($date2_month < 10)) {
        ...
        if (($date3_month > 0) && ($date3_month < 10)) {
            $date3_month = '0' . $date3_month;
        }
