...
class EditionAlms extends Model
{
	...
	public function modEdition()
	{
		...
		$retVal = $this->edition_man->modEdition($this->id_edition, $edition_info['code'], $edition_info['name'], $edition_info['description'], $edition_info['status'], $edition_info['max_par'], $edition_info['min_par'], $edition_info['price'], $edition_info['date_begin'], $edition_info['date_end'], $edition_info['overbooking'], $edition_info['can_subscribe'], $edition_info['sub_date_begin'], $edition_info['sub_date_end']);
		
		//ABR: aggiorno stato assegnazioni
		$retVal = $this->courseassn_man->updAssnStatus($this->id_course, $this->id_edition, $edition_info['status'], 'elearning');
	...
	{
		//Elimino l'edizione
		$retVal = $this->edition_man->delEdition($this->id_edition);
		
		//ABR: Aggiorno eventuali assegnazioni
		if($retVal)
			$retVal = $this->courseassn_man->updAssnEditionExistent($this->id_course, $this->id_edition);
