...
class CourseAlms extends Model
{
    ...
    public const boxDescrMaxLimit = 500; //ABR
    ...
    {
		//ABR: spostata nella lib
		return $this->course_man->getCategoryDescendants($id_category);
    }
    ...
    public function getCourseModDetails($id_course = false)
    {
        if ($id_course === false) {
            return [
                ...
                'new_content_indicator_days' => Formalms\lib\Get::Sett('new_content_indicator_days', 0),
            ];
        } else {
    ...
    public function insCourse($data_params = null)
    {
        ...
        SET idCategory          = '" . (isset($data_params['idCategory']) ? $data_params['idCategory'] : 0) . "',
            ...
            course_virtual		= '" . (int) $data_params['course_virtual'] . "',
			new_content_indicator_days = '" . (int) $data_params['new_content_indicator_days'] . "',
            ...
            in_home_catalogue   = '" . (isset($data_params['course_in_home_catalogue']) ? 1 : 0) . "',
            ...
            prize               = '" . str_replace(',', '.', str_replace('.', '', $data_params['course_prize'])) . "',
    ...
    public function upCourse($id_course = null, $data_params = null)
    {
        ...
        SET code                = '" . $data_params['course_code'] . "',
            ...
            selling             = '" . (isset($data_params['course_sell']) ? 1 : 0) . "',
            prize               = '" . (isset($data_params['course_prize']) ? str_replace(',', '.', str_replace('.', '', $data_params['course_prize'])) : 0) . "',
    ...
    public function delCourse($id_course)
    {
        ...
        //    WHERE idCourse = '" . (int) $id_course . "'";
  		...
  		// ABR: Fix Query.
        $query_course = "SELECT imgSponsor, img_course, img_material, img_othermaterial, course_demo, course_type, (COUNT(id_edition) > 0) AS has_editions  
							FROM %lms_course c
								LEFT JOIN %lms_course_editions e ON c.idCourse = e.id_course 
							WHERE idCourse = '" . (int)$id_course . "' 
							GROUP BY idCourse";      
        ...
        if ($course_type == 'classroom') {
            ...
            //ABR: elimino informazioni fondi su edizioni (no ciclo, complessive per progetto)
            (new FundManager('date'))->delFundEntryByCourse($id_course);
        ...
        //--- ABR: elimino assegnazioni---------------------------------------------
        require_once(_lms_ . '/lib/lib.courseassn.php');
        (new CourseassnManager)->updAssnCourseDeleted($id_course);
        //--- fine elimino assegnazioni---------------------------------------------
