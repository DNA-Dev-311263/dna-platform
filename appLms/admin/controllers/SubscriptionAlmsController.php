...
class SubscriptionAlmsController extends AlmsController
{
    ...
    public function add()
    {
        ...
        if (isset($_POST['okselector'])) {
                ...
                case 'students':
                        ...
                        if ($can_subscribe) {
							...
							if(!empty($user_selected))
								$this->_raiseSubsEvent($user_selected, $id_course, $edition, _EVENT_SUBSCRIPTION_INSERT);
                            ...
                            if (!empty($user_selected) && $send_alert) {
								...
								if($edition) {
									//ABR gestione modificata per invio mail se corso a edizioni															
									$this->_sendSubsAlert($user_selected, $course_info, $edition);			
									
								} else {
									//ABR codice FORMALMS per invio mail, usato solo per corsi senza edizioni.
									// id_date non ci sarà...							
            ...
            $this->render('level', [
                ...
                'send_alert' => FormaLms\lib\Get::req('send_alert', DOTY_INT, 0),
                'send_calendar' => $course_info['sendCalendar'], //ABR
    ...
    public function ins()
    {
        ...
        if ($can_subscribe) {
            ...
            if (!empty($user_selected) && $send_alert) {
				...
				if($edition) {
					//ABR gestione modificata per invio mail se corso ad edizioni																	
					$this->_sendSubsAlert($user_selected, $course_info, $edition);			
            ...
            if ($_POST['subs']) {
                ...
                foreach ($subs as $sub) {
                    ...
                    // message to user that is modified
    ...
    public function del()
    {
        ...
        if (!$this->permissions['subscribe_course']) {
            ...
            return;
		}elseif ($this->model->checkEditionFinished()){
			//mod. ABR
			$output = array('success' => false, 'message' => $this->_getMessage("edition closed"));
			echo $this->json->encode($output);
			return;				
		}
        ...
        if ($this->model->delUser($id_user)) {
			...
			// ABR: sollevo evento
			$edition = ($this->id_edition ? $this->id_edition : $this->id_date);
			$this->_raiseSubsEvent( array($id_user => $level), $this->id_course, $edition, _EVENT_SUBSCRIPTION_DELETE );
				
			// ABR: Invio segnalazione rimozione iscrizione
			$this->_sendUnsubscribeAlert(array($id_user));
    ...
    public function multidel()
    {
        ...
        if ($users == '') {
            ...
            foreach ($list_users as $id_user) {
                ...
                if ($this->model->delUser($id_user)) {
                    ...
                    $deleted[ $id_user ] = $level; //ABR dic-21
		...
		// ABR: sollevo evento
		$edition = ($this->id_edition ? $this->id_edition : $this->id_date);
		$this->_raiseSubsEvent($deleted, $this->id_course, $edition, _EVENT_SUBSCRIPTION_DELETE);
		
		// ABR: invio segnalazioni rimozione iscrizione
		$this->_sendUnsubscribeAlert($deleted);
	...
	public function update()
	{
		...
		switch ($col) {
			...
			case 'level':
				...
				if (!isset($level_idst[$level]) || !isset($level_idst[$new_value])) {
					break;
				}
				...
				$result['success'] = $this->model->updateUserLevel($id_user, $new_value);
				break;
			...
			case 'status':
				...
				if ($this->model->updateUserStatus($id_user, $new_value)) {
					...
					Events::trigger(
						'core.course.subscription.status.updated',
						[
							'user'   => $user,
							'status' => ['id' => $new_value, 'name' => $status[$new_value]],
							'course' => $docebo_course->course_info
						]
					);
				}
			...
			case 'date_begin':
				$result['success'] = $this->model->updateUserDateBeginValidity(
					$id_user,
					Format::dateDb($new_value, 'date')
				);
				break;


			case 'date_expire':
				$result['success'] = $this->model->updateUserDateExpireValidity(
					$id_user,
					Format::dateDb($new_value, 'date')
				);
				break;
		...
		//ABR: sollevo evento
		if ($result['success'])
			$this->_raiseSubsEvent(array($id_user), $this->id_course, 0, _EVENT_SUBSCRIPTION_UPDATE, array($col => $new_value));
    ...
    public function fastsubscribe()
    {
        ...
        if (isset($admin_users) && array_search($id_user, $admin_users, false) === false && Docebo::user()->getUserLevelId() !== ADMIN_GROUP_GODADMIN) {
            ...
            if ($result) {
				...
				//ABR: Info edizione, utente 
				$edition = ($this->id_date > 0 ? $this->id_date : $this->id_edition);
				$user_selected = array($id_user => $level);
			
				//ABR: sollevo evento
				$this->_raiseSubsEvent($user_selected, $this->model->getIdCourse(), $edition, _EVENT_SUBSCRIPTION_INSERT);
            ...
            if ($set_level <= 0 && $set_status <= 0 && $set_date_begin <= 0 && $set_date_expire <= 0 && $reset_date_begin <= 0 && $reset_date_expire <= 0) {
                ...
                if ($set_level > 0) {
                    ...
                    if ($new_level > 0) {
                        ...
                        if( $res1 ) $action_details['level'] = $new_level; //ABR
                    }
                }
                ...
                if ($set_status > 0) {
                    ...
                    if (in_array($new_status, array_keys($this->model->getUserStatusList()))) {
                        ...
                        if( $res2 ) $action_details['status'] = $new_status; //ABR
                    }
                ...
                if ($set_date_begin > 0) {
					...
					if( $res3 ) $action_details['date_begin'] = $new_date_begin;
                }
                ...
                if ($set_date_expire > 0) {
					...
					if( $res4 ) $action_details['date_expire'] = $new_date_expire; //ABR
                }

                $res5 = true;
                if ($reset_date_begin > 0) {
                    $res5 = $sman->resetValidityDateBegin($this->id_course, 0, $users_list); //$this->id_edition
                    if( $res5 ) $action_details['reset_date_begin'] = true; //ABR
                }

                $res6 = true;
                if ($reset_date_expire > 0) {
                    $res6 = $sman->resetValidityDateExpire($this->id_course, 0, $users_list); //$this->id_edition
                    if( $res6 ) $action_details['reset_date_expire'] = true; //ABR
                }
                
				//ABR: Sollevo evento (N.B. Le modifiche sono fatte a livello di corso e non di edizione)
				if ($action_details)
					$this->_raiseSubsEvent($users_list, $this->id_course, 0, _EVENT_SUBSCRIPTION_UPDATE, $action_details);
    ...
    /** ABR **
     *  Esegue il reset delle informazioni di fruizione dell'utente
     */
	public function multireset() {
		...
		}elseif ($this->model->checkCourseClosed()) {
			$output = array('success' => false, 'message' => $this->_getMessage("course closed"));
			echo $this->json->encode($output);
			return;		
		}
		...
		if ($users == '') {
			...
			foreach ($list_users as $id_user) {
				if ($this->model->resetCourseUserTrackInfo($id_user)) {
					$count++;
					$reset[] = $id_user;
				}
			}
			
			$output = array('success' => true, 'count' => $count, 'total' => $total, 'reset' => $reset);
		}
	 ...
	 */
	private function _sendUnsubscribeAlert($user_selected) {
		
		require_once(_lms_.'/lib/lib.course.php');
		require_once(_lms_.'/lib/lib.date.php');
		require_once(_lms_.'/lib/lib.courseassn.php');
		
		// Istanzio manager e composer
		$course_man = new Man_Course();
		...
		if ($edition > 0) {
			...
			foreach ($user_selected as $id_user) {
				...
				$ref_info = $courseassn_man->getUserReferent($id_user, $course_info['id_course']); 
				...
				createNewAlert(	'UserCourseRemoved', 'unsubscribe', 'insert', '1', 'User unsubscribed', $recipients, $msg_composer );
	...
	/** ABR **
	 * Invia l'email di segnalazione quando l'edizione raggiunge il numero massimo di iscrizioni
	 */
	private function _sendEditionFullAlert($course_info) {
		...
		$array_subst = array(	'[url]' => FormaLms\lib\Get::sett('url'),
								'[course]' => $course_info['name'],
								'[edition_code]' => $course_info['code'],
								'[date_begin]' => Format::datetimeToString($course_info['date_begin'], 'datetime'),
								'[date_end]' => Format::datetimeToString($course_info['date_end'], 'datetime')
							);
	...
	/** ABR **
	 * Invia l'email di conferma a seguito dell'iscrizione
	 * Le mail devono partire con i seguenti metodi: add, ins, fastadd.
	 */
	private function _sendSubsAlert(array $user_subscribed, array $course_info, int $id_edition) {
		...
		foreach($user_subscribed as $id_user => $level) {
			...
			$array_subst = array(	'[url]' => FormaLms\lib\Get::sett('url'),
									...
									'[level]' => strtolower(Lang::t('_LEVEL_'.$level, 'levels')),
			...
			// Rimuovo tag non pertinenti con livello
			$patterns[3] = array('/<teacher>(.*?)<\/teacher>/s', '/<admin>(.*?)<\/admin>/s'); 		//studente
			$patterns[6] = array('/<student>(.*?)<\/student>/s', '/<admin>(.*?)<\/admin>/s'); 		//docente
			$patterns[7] = array('/<student>(.*?)<\/student>/s', '/<teacher>(.*?)<\/teacher>/s'); 	//amministratore
			
			$pattern_remove = (isset($patterns[$level]) ? $patterns[$level] : $patterns[3]);
			...
			if ($level <= 3) {
				...
				// Livello superiore (tutor, docente, admin ecc. Non inserisco i referenti)
            ...
            createNewAlert('UserCourseInserted', 'subscribe', 'insert', '1', 'User subscribed', $recipients, $msg_composer, false);
         
            
 			// Invio comunicazione per ogni utente iscritto
			//createNewAlert(	'UserCourseInserted', 'subscribe', 'insert', '1', 'User subscribed',
			//				$recipients, $msg_composer, false, false );
	...
	/** ABR **
	 * Solleva evento per eventuali classi in ascolto
	 */
	private function _raiseSubsEvent($users, $id_course, $id_edition, $action, $action_details = null) {
		
		if ($action == _SUBS_UPDATE && $id_edition == 0 ) {
			// Le modifiche iscrizione avvengono in forma a livello di corso e non di edizione (es. stato utente)
			// Riporto comunque l'id edizione su cui è stata richiesta la modifica
			
			$edition['d'] = FormaLms\lib\Get::req('id_date', DOTY_INT, 0);
			$edition['e'] = FormaLms\lib\Get::req('id_edition', DOTY_INT, 0);
			$id_edition = ($edition['d'] ? $edition['d'] : $edition['e']);
		}
		
		// Unisco ai dettagli il modulo di provenienza della chiamata
		$from = array('modActFrom' => 'alms');
		$action_details = is_array($action_details) ? array_merge($action_details, $from) : $from;
    ...
    public function searchusersubscriptions()
    {
        if (!$this->permissions['subscribe_course']) {
            echo $this->json->encode(['success' => false, 'message' => 'Permesso negato']);
            return;
        }

        $raw     = FormaLms\lib\Get::req('q', DOTY_STRING, '');
        $escaped = $this->db->escape($raw);
        $q       = '%' . addcslashes($escaped, '%_') . '%';

        $query = 'SELECT u.idst, u.userid, u.firstname, u.lastname,'
            . ' c.idCourse, c.name AS course_name,'
            . ' cu.status, cu.date_inscr'
            . ' FROM %adm_user AS u'
            . ' JOIN %lms_courseuser AS cu ON u.idst = cu.idUser'
            . ' JOIN %lms_course AS c ON cu.idCourse = c.idCourse'
            . " WHERE u.userid LIKE '" . $q . "'"
        ...
        $status_map = [
            -2 => ['label' => "Lista d'attesa", 'bg' => '#e2e8f0', 'color' => '#4a5568'],
            -1 => ['label' => 'Da confermare',  'bg' => '#fff3cd', 'color' => '#856404'],
             0 => ['label' => 'Iscritto',        'bg' => '#d4e6ff', 'color' => '#1a56a0'],
             1 => ['label' => 'In corso',         'bg' => '#ffe8cc', 'color' => '#7a4200'],
             2 => ['label' => 'Completato',       'bg' => '#d4edda', 'color' => '#1e7e34'],
             3 => ['label' => 'Sospeso',          'bg' => '#f8d7da', 'color' => '#721c24'],
             4 => ['label' => 'Overbooking',      'bg' => '#e2e8f0', 'color' => '#4a5568'],
        ];

        while ($row = $this->db->fetch_assoc($res)) {
            $st  = (int) $row['status'];
            $cfg = isset($status_map[$st]) ? $status_map[$st]
                 : ['label' => 'Sconosciuto', 'bg' => '#e2e8f0', 'color' => '#4a5568'];

            $badge = '<span style="background:' . $cfg['bg'] . ';color:' . $cfg['color']
                   . ';padding:3px 9px;border-radius:12px;font-size:11px;font-weight:600;">'
                   . htmlspecialchars($cfg['label']) . '</span>';

            if ($user_label === null) {
                $user_label = trim(htmlspecialchars($row['firstname']) . ' ' . htmlspecialchars($row['lastname']))
                            . ' (' . htmlspecialchars($row['userid']) . ')';
            }

            $rows[] = [
                'id_user'         => (int) $row['idst'],
                'id_course'       => (int) $row['idCourse'],
                'course_name'     => htmlspecialchars($row['course_name']),
                'status_badge'    => $badge,
                'date_subscribed' => ($row['date_inscr'] && $row['date_inscr'] !== '0000-00-00')
                                   ? date('d/m/Y', strtotime($row['date_inscr']))
                                   : '—',
            ];
        }
    ...
    public function deletesubscription()
    {
        ...
        if (!$id_user || !$id_course) {
            echo $this->json->encode(['success' => false, 'message' => 'Parametri non validi']);
            return;
        }
        ...
        $this->model     = new SubscriptionAlms($id_course, 0, 0);
        ...
        if ($this->model->delUser($id_user)) {
            if ($level !== null && isset($level_idst[$level])) {
                $this->acl_man->removeFromGroup($level_idst[$level], $id_user);
            }
            echo $this->json->encode(['success' => true]);
        } else {
            echo $this->json->encode([
                'success' => false,
                'message' => "Impossibile cancellare l'iscrizione",
            ]);
        }
    }
    ...
    {
        if (!$this->permissions['subscribe_course']) {
            Util::jump_to('index.php?r=' . $this->link . '/show&id_course=' . $this->id_course);
            return;
        }
        ...
        $fmt = function ($value) use ($delimiter) {
            $value = str_replace($delimiter, $delimiter . $delimiter, (string) $value);
            if (strpbrk($value, ",\"\r\n") !== false) {
        ...
        $head = ['Username', 'Nome', 'Cognome', 'Email', 'Stato', 'Data iscrizione', 'Data completamento'];
        $output = implode($separator, array_map($fmt, $head)) . $line_end;

        $query = 'SELECT u.userid, u.firstname, u.lastname, u.email,'
               . ' cu.status, cu.date_inscr, cu.date_complete'
               . ' FROM %adm_user AS u'
               . ' JOIN %lms_courseuser AS cu ON u.idst = cu.idUser'
               . ' WHERE cu.idCourse = ' . (int) $this->id_course
        ...
        while ($row = $this->db->fetch_assoc($res)) {
            ...
            $row_data = [
                ...
                $row['lastname'],
                $row['email'],
                isset($status_map[$st]) ? $status_map[$st] : 'Sconosciuto',
                ($row['date_inscr'] && $row['date_inscr'] !== '0000-00-00' && strtotime($row['date_inscr']))
                    ? date('d/m/Y', strtotime($row['date_inscr'])) : '',
                ($row['date_complete'] && $row['date_complete'] !== '0000-00-00' && strtotime($row['date_complete']))
                    ? date('d/m/Y', strtotime($row['date_complete'])) : '',
            ];
            $output .= implode($separator, array_map($fmt, $row_data)) . $line_end;
        }

        sendStrAsFile($output, 'iscritti_corso_' . (int) $this->id_course . '_' . date('Ymd') . '.csv');
    }
}
