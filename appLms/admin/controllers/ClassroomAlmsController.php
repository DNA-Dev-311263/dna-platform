...
class ClassroomAlmsController extends AlmsController
{
    ...
    public function init()
    {
        ...
        require_once(_base_.'/lib/lib.eventmanager.php'); 	//ABR
    ...
    protected function classroom()
    {
        ...
        //ABR Sollevo evento visualizzazione edizioni classroom
        Events::trigger('lms.edition.list', [ 'id_course' => $this->idCourse ]);
    }
    ...
    public function addClassroom()
    {
        ...
        if (isset($_POST['save'])) {
            if ($idDate = $this->model->saveNewDate()) {
				...
				//ABR: Sollevo evento inserimento nuova edizione
				$this->_raiseEditionEvent($this->idCourse, $this->idDate, _EVENT_EDITION_INSERT);
        ...
        $this->render('classroom', [
            ...
            'availableFund' => $this->model->getFundForDropdown(),
        ]);
    }
    ...
    public function updateClassroom()
    {
        ...
        if (isset($_POST['save'])) {
			...
			//ABR: Recupero info prima dell'aggiornamento
			$old_date_info = $this->model->getDateInfo();
		
            if ($this->model->updateDate()) {
				
					//ABR: Azioni post aggiornamento
					$new_status = FormaLms\lib\Get::req('status', DOTY_INT, 0);
					
					// Invio mail al cambiamento di stato su cancellato
					if ($old_date_info['status'] != 2 && $new_status == 2) {
						$this->_sendCancelEditionAlert($this->idCourse, $this->idDate);
					}
					
					//ABR: Sollevo evento modifica dati edizione
					$this->_raiseEditionEvent($this->idCourse, $this->idDate, _EVENT_EDITION_UPDATE, [
							'previous_record' => $old_date_info, 
							'new_record' => $this->model->getDateInfoFromPost()
					]);	
								
				// Porto utente su pagina edizioni
        ...
        $this->render('classroom',
            [
                ...
                'postData' => [
                    ...
                    'internal_note' => FormaLms\lib\Get::req('internal_note', DOTY_STRING, $dateInfo['internal_note']),	//ABR
                    'id_fund' => FormaLms\lib\Get::req('id_fund', DOTY_STRING, $dateInfo['id_fund']),					//ABR
                    'fund_info_exists'=> $dateInfo['fund_info_exists'],													//ABR
    ...
    public function classroomDateDays()
    {
        ...
        $sendAlert = (bool) FormaLms\lib\Get::pReq('sendAlert', DOTY_BOOL, false); //ABR

        if ($_SERVER['REQUEST_METHOD'] === 'POST') {

			//ABR: sollevo evento
			$this->_raiseEditionEvent($this->idCourse, $this->idDate, _EVENT_EDITION_DAY_BEFORE_UPDATE, $postData);	
            ...
            //ABR: sollevo evento
            $this->_raiseEditionEvent($this->idCourse, $this->idDate, _EVENT_EDITION_DAY_AFTER_UPDATE);	
            ...
            if ($result) {
                ...
                // Mod. ABR Invio aggiornamento pianificazione
                if ($sendAlert)
        ...
        //ABR: Recupero corso e tipo
        $courseInfo = $this->model->getCourseInfo();
        $virtual = (bool)$courseInfo['course_virtual'];
        ...
        $this->render('classroom-dates',
            [
                ...
                'availableStatuses' => $this->model->getStatusForDropdown(),
                'availableTestTypes' => $this->model->getTestTypeForDropdown(),
                'availableClassrooms' => $this->model->getClassroomForDropdown($virtual), 	//ABR
                'eventCalendarChangeState' => $this->model->getEventCalendarChangeState(), 	//ABR
            ]
        );
    }
    ...
    */
    ...
    /** ABR DEV
     * Invia il calendario agli utenti già iscritti. Se richiesto, anche l'allegato .ics
     */
    private function sendCalendarToAllSubscribers()
    {
		
		// Recupero informazioni sul corso
		$course_info = $this->model->getCourseInfo();
		...
		// Istanzio manager e altri model
		...
		// - Info giorni, sedi		
		$days = $this->model->getDateDaysAndLocations();
		...
		foreach($users as $user) {
			...
			$msg_composer->setSubjectLangText('email', '_COURSE_CHANGE_CALENDAR_SUBJECT', ['[course]' => $course_info['name']]);
			...
			$array_subst = array(	'[url]' => FormaLms\lib\Get::sett('url'),
									...
									'[level]' => strtolower(Lang::t('_LEVEL_'.$user['level_id'], 'levels'))
								);
								
			// Recupero testo body
			$body =  Lang::t('_COURSE_CHANGE_CALENDAR_HTML', 'email', $array_subst);
			...
			if ($level <= 3) {
				...
				$ref_info = $courseassn_man->getUserReferent($id_user, $this->idCourse, $this->idDate);
            ...
            if ($course_info['sendCalendar']) {
                ...
                $calendar = \CalendarManager::getCalendarDataContainerForDateDays($this->idCourse, $this->idDate, $uinfo[ACL_INFO_IDST]);
                $msg_composer->setAttachments([$calendar->getFile()]);
            }
            
		
			// Invio comunicazione per ogni utente iscritto
            createNewAlert('ClassroomCalendarChange', 'calendar', 'update', '1', 'Calendar changed', $recipients, $msg_composer, false);
    ...
    protected function delclassroom()
    {
        ...
        //ABR: Sollevo evento eliminazione edizione
        if ($res)
			$this->_raiseEditionEvent($this->idCourse, $this->idDate, _EVENT_EDITION_DELETE);
    ...
    public function getPdf($html, $name, $img = false, $orientation = 'L', $download = true, $facs_simile = false, $for_saving = false)
    {
     ...
     /** ABR **
     * Restituisce caption delle caselle di intput sul finanziamento
     * Se id_date è valorizzato, il metodo inserisce gli eventuali valori già presenti
     * Chia
     */
	public function getFundEntryInput() {
		$id_fund = FormaLms\lib\Get::req('id_fund', DOTY_INT, false);
		$id_date = FormaLms\lib\Get::req('id_date', DOTY_INT, false);
		
		$res = $this->model->getFundEntryInput($id_fund, $id_date);
    ...
    /** ABR **
     * Invia l'email di segnalazione per cancellazione edizione - cambiamento di stato
     */
	private function _sendCancelEditionAlert($id_course, $id_date) {
		
		// Recupero modello
		$model = new ClassroomAlms($id_course, $id_date);
		
		// Recupero manager
		$classroom_man = $model->classroom_man;
		$courseassn_man = $model->courseassn_man;
		...
		// Proprietà oggetto e-mail
		$msg_composer->setSubjectLangText('email', '_EDITION_CANCELED_SUBJECT', false);
			
		// Preparo array per sostituzione tag della comunicazione
		$array_subst = array(	'[url]' => FormaLms\lib\Get::sett('url'),
								'[course]' => $date_info['name'],
								'[edition_code]' => $date_info['code'],
								'[date_begin]' => Format::datetimeToString($date_info['date_begin'], 'date'),
								'[date_end]' => Format::datetimeToString($date_info['date_end'], 'date')
							);
								
		// Proprietà corpo dell'e-mail
		$msg_composer->setBodyLangText('email', '_EDITION_CANCELED_HTML', $array_subst);
			
			
		// Recupero iscritti
		$users = $model->getSubscribedUsers();
				
			
		foreach ($users as $id_user) {
			// Destinatari 
			
			$ref_info = $courseassn_man->getUserReferent($id_user, $id_course, $id_date);
			...
			if($ref_info['idst'][0])
				$recipients[] = $ref_info['idst'][0];
				
			// - Responsabili corso dell'organizzazione dell'utente
			if($ref_info['idst'][1] != "")
				$recipients =  array_merge($recipients, explode(" ", $ref_info['idst'][1]));
			
		}
		
		// Prendo i referenti manager una sola volta
		$recipients = array_unique($recipients);
		
		
		// Aggiungo super amministratori
		$levels_id = $this->acl_man->getAdminLevels();
		$godadmins = $this->acl_man->getGroupUMembers($levels_id[ADMIN_GROUP_GODADMIN]);
		
		$recipients = array_merge($recipients, $godadmins);

		
		// Invio alert
		createNewAlert(	'EditionCanceled', 'edition canceled', 'update', '1', 'Edition canceled', $recipients, $msg_composer );
	}
	
	
	private function _sendPresenceRegister($id_course, $id_date) {
		//>> ABR: Invia l'email con il report sulle presenze del corso
		
		//Recupero la data e ora corrente
		$now = date('Y-m-d H:i:s'); 		//date("Y-m-d H:i:s", strtotime('03/06/2019'));
		
		// Recupero modello classroom
		$model = new ClassroomAlms($id_course, $id_date);

		// Istanzio evento
		$msg_composer = new EventMessageComposer();
		
		// Recupero manager assegnazioni
		$courseassn_man = $model->courseassn_man;
		
		// Recupero info edizione
		$date_info = $model->getDateAllInfo();
	
		
		// Proprietà oggetto e-mail
		$msg_composer->setSubjectLangText('email', '_COURSE_PRESENCE_SUBJECT', false);
		
		// Recupero la tabella da inviare
		$table = Util::removeHtmlTag($model->getPresenceTable(), 'a');
		
		// Aggiungo la classe per la formattazione della data corrente
		$table = str_replace(Format::datetimeToString($now, 'date'),'<span class="today">'.Format::datetimeToString($now, 'date').'</span>', $table);
		...
		$array_subst = array(	'[url]' => FormaLms\lib\Get::sett('url'),
								'[course]' => $date_info['name'],
								'[edition_code]' => $date_info['code'],
								'[teachers]' => $date_info['teachers'],
								'[classrooms]' => $date_info['classrooms'],
								'[date_begin]' => Format::datetimeToString($date_info['date_begin'], 'date'),
								'[date_end]' => Format::datetimeToString($date_info['date_end'], 'date'),
								'[now]' => Format::datetimeToString($now, 'datetime'),
								'[presence_table]' => $table
							);
								
		// Proprietà corpo dell'e-mail
		$msg_composer->setBodyLangText('email', '_COURSE_PRESENCE_HTML', $array_subst);
		...
		foreach ($users as $id_user) {
			...
			// - Prendo solo i responsabili corso (organizzazione dell'utente)
			if($ref_info['idst'][1] != "")
				$recipients =  array_merge($recipients, explode(" ", $ref_info['idst'][1]));
		}
		
		// Prendo i responsabili una sola volta
		$recipients = array_unique($recipients);
		
		
		// Invio alert
		createNewAlert(	'CoursePresenceSaved', 'presence', 'update', '1', 'Course presence saved', $recipients, $msg_composer );
	}
	...
	**/
	private function _raiseEditionEvent($id_course, $id_edition, $action, $action_details = null) {
		
		 Events::trigger('lms.edition.manage', [
			'id_course'      => $id_course,
