...
define('ORGFIELD_VIDEOCOMPLETE', 26); //ABR
...
class OrgDirDb extends RepoDirDb
{
    ...
    public function _getOtherFields($tname = false)
    {
        ...
        if ($tname === false) {
            return $parent . ', idCourse, prerequisites,'
                . ' isTerminator, idParam, visible, milestone, width, height, publish_from, publish_to, access, publish_for, ignoreScore, videoComplete '; //ABR
        } else {
            return $parent . ', '
                . $tname . '.idCourse,'
                . $tname . '.prerequisites,'
    ...
    public function _getOtherValues()
    {
        return parent::_getOtherValues() . ", '"
            ...
            . (int) $this->org_videoComplete . ' '; //ABR
    }
    ...
    public function _getOtherUpdates()
    {
        return parent::_getOtherUpdates() . ', '
            ...
            . ' videoComplete=' . (int) $this->org_videoComplete . ' ';
    }
    ...
    public function renameFolder(&$folder, $newName)
    {
        ...
        $this->org_videoComplete = $folder->otherValues[ORGFIELD_VIDEOCOMPLETE]; //ABR
    ...
    public function modifyItem($arrData, $idCourse = false, $strips = false)
    {
        ...
        $this->org_videoComplete = isset($arrData['videoComplete']) //ABR
            ? $arrData['videoComplete']
            : $folder->otherValues[ORGFIELD_VIDEOCOMPLETE];
...
class Org_TreeView extends RepoTreeView
{
    ...
    public function getLoData($idLoList)
    {
        ...
        foreach ($idLoList as $index => $idLo) {
            ...
            if (!$node['is_folder']) {
                ...
                //ABR, calcolo lo stato di avanzamento della cartella
                $node['status'] = $this->getFolderStatus($folder);
    ...
    /**
     * ABR
     * Restituisce lo stato di avanzamento complessivo della cartella passata in argomento
     * result: 0 non iniziato, 1 iniziato, 2 finito
     */
    public function getFolderStatus($folder):int
	{
		$res = 0;
		$passed_counter = 0;
		$attempted_counter = 0;
		$total_content = 0;
		$lo_ids = $this->tdb->getDescendantsId($folder);

		if (!$lo_ids) {
			return $res;
		}

		foreach ($lo_ids as $id) {
			$child = $this->tdb->getFolderById($id);
			if (!$child || $child->otherValues[REPOFIELDOBJECTTYPE] === '') {
				continue;
			}
			$total_content++;
			$status = Track_Object::getStatusFromId($id, getLogUserId());
			switch ($status) {
				case 'passed':
				case 'completed':
					$passed_counter++;
		...
		if ($total_content > 0 && $passed_counter === $total_content) {
			$res = 2;
		} elseif ($passed_counter > 0 || $attempted_counter > 0) {
			$res = 1;
		}
