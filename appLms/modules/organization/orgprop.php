...
function organization_property_prereqisites(&$treeView, $idItem, &$form, &$lang)
{
    ...
    $GLOBALS['page']->add(Form::getHidden('videoComplete', 'videoComplete', $values['videoComplete'])); //ABR
}
...
function organization_property_settings(&$treeView, $idItem, &$form, &$lang)
{
        ...
        if ($arrParamsInfo !== false) {
            ...
            if (is_callable([$lo, 'renderCustomSettings'])) {
                ...
                while ($param = current($arrParamsInfo)) {
                    ...
                    } elseif (isset($param_values[$param['param_name']])) {
    ...
    //------------------- video complete
    //ABR opzione completamento oggetto al progress video (per htmlpage)  
    if (isset($lo) && get_class($lo) == 'Learning_Htmlpage') {
        $GLOBALS['page']->add(Form::getNumberfield($lang->def('_VIDEO_COMPLETE'),
                                                'videoComplete',
                                                'videoComplete',
                                                0, 100,
                                                isset($values['videoComplete']) ? $values['videoComplete'] : '')
                            );		
...
function organization_property_common(&$treeView, $idItem, &$form, &$lang, &$folder)
{
    ...
    if (isset($_POST['stay_on_properties'])) {
        ...
        $values['videoComplete'] = $_POST['videoComplete']; //ABR
    } else {
        ...
        $values['videoComplete'] = $data[ORGFIELD_VIDEOCOMPLETE]; //ABR
    }
