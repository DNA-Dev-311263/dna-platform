...
function loadUnreaded()
{
    ...
    if ($unreadedForum && !isset($unreadedForum[$id_course])) {
...
function forum()
{
    ...
    if (FormaLms\lib\Get::sett('forum_as_table') == 'on') {
        ...
        while (list($idF, $title, $descr, $num_thread, $num_post, $locked, $emoticons) = sql_fetch_row($re_forum)) {
            ...
            } elseif ($unreadedForum && isset($unreadedForum[$idCourse][$idF])) {
                if (isset($unreadedForum[$idCourse][$idF]) && is_array($unreadedForum[$idCourse][$idF])) {
                    foreach ($unreadedForum[$idCourse][$idF] as $k => $n_mess) {
            ...
            if ($mod_perm) {
                if ($i != $tot_forum) {
                    $content[] = '<a href="index.php?modname=forum&amp;op=downforum&amp;idForum=' . $idF . '">
                ...
                if ($i != 1) {
                    $content[] = '<a href="index.php?modname=forum&amp;op=moveupforum&amp;idForum=' . $idF . '">
        ...
        while (list($idF, $title, $descr, $num_thread, $num_post, $locked, $emoticons) = sql_fetch_row($re_forum)) {
            ...
            if ($locked) {
                $status = '<span class="ico-sprite subs_locked"><span>' . Lang::t('_LOCKED', 'forum') . '</span></span>';
            } elseif (isset($unreadedForum[$idCourse][$idF])) {
                if (isset($unreadedForum[$idCourse][$idF]) && is_array($unreadedForum[$idCourse][$idF])) {
                    foreach ($unreadedForum[$idCourse][$idF] as $k => $n_mess) {
                        if ($n_mess != 'new_thread') {
                            $mess_notread += $n_mess;
            ...
            if ($mod_perm) {
                ...
                $GLOBALS['page']->add(
                    ...
                    ' title="' . $lang->def('_DEL') . ' : ' . strip_tags($title) . '">
						<img src="' . getPathImage() . 'standard/delete.png" alt="' . $lang->def('_DEL') . '" /></a></li>', 'content');
...
function thread()
{
    ...
    if ($idEdition) {
        $query_thread .= " AND id_edition = '" . $idEdition . "'";
    }
    ...
    while (list($idT, $t_author, $posted, $title, $num_post, $num_view, $locked, $erased, $important, $isPrivate) = sql_fetch_row($re_thread)) {
        ...
        if ((int) $isPrivate === 0 || ((int) $isPrivate === 1 && ($t_author === $currentUserId || $authorIsAdmin) || $moderate)) {
            ...
            if (isset($unreadedForum[$idCourse][$id_forum][$idT]) && $unreadedForum[$idCourse][$id_forum][$idT] != 'new_thread') {
                $unread_message = $unreadedForum[$idCourse][$id_forum][$idT];
            ...
            if ($erased) {
                ...
                $status = '<span class="ico-sprite subs_locked"><span>' . Lang::t('_LOCKED', 'forum') . '</span></span>';
            } elseif (isset($unreadedForum[$idCourse][$id_forum][$idT])) {
                $status = '<img src="' . getPathImage() . 'standard/msg_unread.png" alt="' . $lang->def('_UNREAD') . '" />';
                $c_css = ' class="text_bold"';
            } else {
            ...
            $unreadedForum = $session->get('unreaded_forum');
            $content[] = $num_post
                . (isset($unreadedForum[$idCourse][$id_forum][$idT]) && $unreadedForum[$idCourse][$id_forum][$idT] != 'new_thread'
                    ? '<br /><span class="forum_notread">(' . $unreadedForum[$idCourse][$id_forum][$idT] . ' ' . $lang->def('_ADD') . ')</span>'
                    : (isset($unreadedForum[$idCourse][$id_forum][$idT]) && $unreadedForum[$idCourse][$id_forum][$idT] == 'new_thread'
    ...
    if (isset($unreadedForum[$idCourse]) && count($unreadedForum[$idCourse])) {
...
function addthread()
{
    ...
    if ($canInsert) {
        $GLOBALS['page']->add(Form::openForm('form_forum', 'index.php?modname=forum&amp;op=insthread', false, false, 'multipart/form-data')
            ...
            . Form::getTextarea($lang->def('_TEXTOF'), 'textof', 'textof'), 'content');
...
function insthread()
{
    ...
    if (!$continue) {
        Util::jump_to('index.php?modname=forum&op=thread&idForum=' . $id_forum . '&amp;result=err_cannotsee');
    }
    if ($locked) {
        Util::jump_to('index.php?modname=forum&op=thread&idForum=' . $id_forum . '&amp;result=err_lock');
    }
	...
	VALUES (
		...
		'" . getLogUserId() . "',
		 ...
		 $isPrivate)";
    if (!sql_query($ins_query)) {
        Util::jump_to('index.php?modname=forum&op=thread&idForum=' . $id_forum . '&amp;result=err_ins');
    }
...
function upthread()
{
    ...
    if (isset($_POST['undo'])) {
        if ($_POST['search'] == 1) {
            Util::jump_to('index.php?modname=forum&op=search&amp;ini=' . $ini);
        } else {
    ...
    if ($_POST['search'] == 1) {
        ...
        Util::jump_to('index.php?modname=forum&op=thread&idForum=' . $id_forum . '&amp;result=ok');
    }
}
    ...
    if (isset($_GET['confirm'])) {
		...
		WHERE idThread = '" . $id_thread . "'")
            ...
            if ($_GET['search'] == 1) {
                ...
                Util::jump_to('index.php?modname=forum&op=thread&idForum=' . $id_forum . '&amp;result=err_del');
            }
        }
        ...
        if ($_POST['search'] == 1) {
            Util::jump_to('index.php?modname=forum&op=search');
        } else {
            Util::jump_to('index.php?modname=forum&op=thread&idForum=' . $id_forum . '&amp;result=ok');
        }
    } else {
...
function message()
{
   ...
   require_once Forma::inc(_base_ . '/lib/lib.user_profile.php');
    ...
    if ($unreadedForum && isset($unreadedForum[$idCourse][$id_forum][$id_thread])) {
        unset($unreadedForum[$idCourse][$id_forum][$id_thread]);
        $session->set('unreaded_forum', $unreadedForum);
        $session->save();
    }
...
function addmessage()
{
    ...
    $GLOBALS['page']->add(
        ...
        . Form::getTextarea($lang->def('_TEXTOF'), 'textof', 'textof', ($id_message != '' ? '<em>' . $lang->def('_WRITTED_BY') . ': ' . $userName . '</em><br /><br />[quote]' . $m_textof . '[/quote]' : '')), 'content');
    if (checkPerm('upload', true)) {
        $GLOBALS['page']->add(Form::getFilefield($lang->def('_UPLOAD'), 'attach', 'attach'), 'content');
    }
    $GLOBALS['page']->add(
...
function insmessage()
{
    ...
    if (!$continue) {
        Util::jump_to('index.php?modname=forum&op=message&idThread=' . $id_thread . '&amp;ini=' . $ini . '&amp;result=err_cannotsee');
    }
...
function delmessage()
{
    ...
    if (isset($_GET['confirm'])) {
		...
		WHERE answer_tree LIKE '" . $answer_tree . "/%' AND idCourse = '" . $idCourse . "'")
        ) {
...
function forumsearchmessage()
{
    ...
    $idCourse = $session->get('idCourse');
    //set as readed if needed
    if (isset($unreadedForum[$idCourse][$id_forum][$id_thread])) {
        unset($unreadedForum[$idCourse][$id_forum][$id_thread]);
        $session->set('unreaded_forum', $unreadedForum);
        $session->save();
    }
