...
if (!Docebo::user()->isAnonymous()) {
	...
	WHERE id_course='" . $idCourse . "'");
        ...
        if (!sql_num_rows($re_file) && !$mod_perm) {
            return '';
        }
    ...
    function infocourse()
    {
        ...
        $GLOBALS['page']->add(
            ...
            . '<tr><th scope="row">' . $lang->def('_LANGUAGE') . '</th><td>' . $course['lang_code'] . '</td></tr>', 'content');

        foreach ($levels as $num_lv => $name_level) {
            if ($course['level_show_user'] & (1 << $num_lv)) {
                $users = &$acl_man->getUsers(Man_Course::getIdUserOfLevel($session->get('idCourse'), $num_lv, $session->get('idEdition')));
                if (!empty($users)) {
                    $first = true;
        ...
        if ($course['show_extra_info'] == '1') {
            $GLOBALS['page']->add(
                ...
                . '<tr><th scope="row">' . $lang->def('_MEDIUMTIME') . '</th><td>' . (int) $course['mediumTime'] . ' ' . $lang->def('_HOURS') . '</td></tr>'
                . '<tr><th scope="row">' . $lang->def('_STATCANNOTENTER') . '</th><td>', 'content');
    ...
    function addfiles()
    {
        ...
        $GLOBALS['page']->add(
            getTitleArea(
                ['index.php?r=lms/course/infocourse' => $lang->def('_INFO'), $lang->def('_ADDFILES')], 'infocourse')
            . '<div class="std_block">'
    ...
    function insfiles()
    {
        ...
        if ($_FILES['attach']['name'] == '') {
            ...
            $savefile = $session->get('idCourse') . '_' . mt_rand(0, 100) . '_' . time() . '_' . $_FILES['attach']['name'];
	...
	WHERE id_course='" . $session->get('idCourse') . "' AND id_file='" . $id_file . "'"));

        $GLOBALS['page']->add(
            getTitleArea(
                ['index.php?r=lms/course/infocourse' => $lang->def('_INFO'), $lang->def('_MOD')], 'infocourse')
            . '<div class="std_block">'
            . getBackUi('index.php?r=lms/course/infocourse', $lang->def('_BACK'))
    ...
    function remfiles()
    {
		...
		WHERE id_course = '" . (int) $session->get('idCourse') . "' AND id_file = '" . (int) $_GET['id_file'] . "'"));
            ...
            $GLOBALS['page']->add(
                getTitleArea(
                    ['index.php?r=lms/course/infocourse' => $lang->def('_INFO'), $lang->def('_DEL')], 'infocourse')
                . '<div class="std_block">'
