...
/**
 * ============================================================================
 * Registro presenze: stessa logica/tabella di Stat. Utilizzo (statistic() +
 * userdetails()), ma raggiungibile dal menu Statistiche del backoffice con
 * scelta esplicita del corso (qui non c'e' un corso "di sessione" come
 * quando si naviga dentro un corso), e senza il grafico di outPageView().
 * ============================================================================
 */

/**
 * Passo 1: scelta del corso.
 */
function registerSelectCourse()
{
    checkPerm('view');
    require_once _lms_ . '/lib/lib.course.php';
    $lang = &DoceboLanguage::createInstance('statistic', 'lms');
    ...
    $course_man = new Man_Course();
    $courses = $course_man->getAllCourses(false, false, false, true);

    $options = '<option value="">-- ' . Lang::t('_SELECT', 'standard') . ' --</option>';
    foreach ($courses as $idCourse => $course) {
        $options .= '<option value="' . (int) $idCourse . '">' . htmlspecialchars($course['name']) . '</option>';
    }

    $GLOBALS['page']->add(getTitleArea($lang->def('_ATTENDANCE_REGISTER')), 'content');
    $GLOBALS['page']->add(
        '<div class="std_block">'
        . '<form method="get" action="index.php">'
        . '<input type="hidden" name="modname" value="statistic" />'
        . '<input type="hidden" name="op" value="registerCourseUsers" />'
        . '<label for="register_course">' . Lang::t('_COURSE', 'standard') . '</label> '
        . '<select name="id_course" id="register_course">' . $options . '</select> '
        . '<input type="submit" class="but_a" value="' . Lang::t('_CONTINUE', 'standard') . '" />'
        . '</form>'
        . '</div>', 'content'
    );
}
...
/**
 * Passo 2: elenco utenti del corso scelto (come statistic(), idCourse
 * esplicito invece che di sessione, niente grafico), con checkbox per
 * l'export multiplo.
 */
function registerCourseUsers()
{
    ...
    $idCourse = importVar('id_course', true, 0);
    if ($idCourse <= 0) {
    ...
    $GLOBALS['page']->add(getTitleArea($lang->def('_ATTENDANCE_REGISTER')), 'content');
    $GLOBALS['page']->add(
        '<div class="std_block"><a href="index.php?modname=statistic&amp;op=registerSelectCourse">&larr; '
        . Lang::t('_BACK', 'standard') . '</a></div>',
        'content'
    );

    $GLOBALS['page']->add(
        '<form method="get" action="index.php" id="register_export_form">'
        . '<input type="hidden" name="modname" value="statistic" />'
        . '<input type="hidden" name="op" value="registerExportSelected" />'
        . '<input type="hidden" name="id_course" value="' . (int) $idCourse . '" />',
        'content'
    );
    ...
    $tb->addHead($cont_h);
    foreach ($users_list as $user_info) {
        $idst = $user_info[ACL_INFO_IDST];
        $cont = [
            '<input type="checkbox" name="selected_users[]" value="' . (int) $idst . '" />',
            '<a href="index.php?modname=statistic&amp;op=registerUserDetails&amp;id_course=' . (int) $idCourse . '&amp;id=' . (int) $idst . '" '
            . 'title="' . $lang->def('_DETAILS') . ' : ' . htmlspecialchars($acl_man->relativeId($user_info[ACL_INFO_USERID])) . '">'
            . htmlspecialchars($acl_man->relativeId($user_info[ACL_INFO_USERID])) . '</a>',
            htmlspecialchars($user_info[ACL_INFO_LASTNAME]),
            htmlspecialchars($user_info[ACL_INFO_FIRSTNAME]),
        ];
        $tb->addBody($cont);
    }

    $GLOBALS['page']->add(getTable($tb, '_USERS_LIST_CAPTION', 'stats_register_users_list'), 'content');

    $GLOBALS['page']->add(
        '<div class="std_block">'
        . '<input type="submit" class="ico-wt-sprite subs_xls but_a" value="' . $lang->def('_EXPORT_ALL_USERS_XLS') . '" />'
        . '</form>'
        . '</div>', 'content'
    );
}
...
/**
 * Passo 3: tabella sessioni dell'utente (come userdetails(), idCourse
 * esplicito, niente grafico/colonna dettaglio sessione).
 */
function registerUserDetails()
{
    ...
    $page_title = [
        'index.php?modname=statistic&amp;op=registerCourseUsers&amp;id_course=' . (int) $idCourse => $lang->def('_ATTENDANCE_REGISTER'),
        ($user_info[ACL_INFO_LASTNAME] . $user_info[ACL_INFO_FIRSTNAME]
            ? $user_info[ACL_INFO_LASTNAME] . ' ' . $user_info[ACL_INFO_FIRSTNAME]
            : $acl_man->relativeId($user_info[ACL_INFO_USERID])),
    ];
    ...
    $query_track = '
	SELECT enterTime, lastTime, (UNIX_TIMESTAMP(lastTime) - UNIX_TIMESTAMP(enterTime)) AS howm,
		numOp, lastFunction, lastOp
	FROM %lms_tracksession
	WHERE idCourse = "' . (int) $idCourse . '" AND idUser = "' . (int) $idst_user . '"
	ORDER BY enterTime';
    $re_tracks = sql_query($query_track);
    ...
    $GLOBALS['page']->add(
        getTitleArea($page_title, 'statistic')
        . getBackUi('index.php?modname=statistic&amp;op=registerCourseUsers&amp;id_course=' . (int) $idCourse, $lang->def('_BACK')),
        'content'
    );

    $tb = new Table(0, $lang->def('_USERS_LIST_DETAILS_CAPTION'), $lang->def('_USERS_LIST_DETAILS_SUMMARY'));
    $type_h = ['', '', 'align_right', 'align_center', ''];
    $cont_h = [
        $lang->def('_SESSION_STARTED'),
        $lang->def('_LAST_ACTION_AT'),
        $lang->def('_HOW_MUCH_TIME'),
        $lang->def('_NUMBER_OF_OP'),
        $lang->def('_LAST_OP'),
    ];
    $tb->setColsStyle($type_h);
    $tb->addHead($cont_h);
    ...
    $session_count = 0;
    $total_seconds = 0;
    while (list($session_start_at, $last_action_at, $how, $num_op, $last_module, $last_op) = sql_fetch_row($re_tracks)) {
        ++$session_count;
        $total_seconds += $how;
        $tb->addBody([
            Format::date($session_start_at),
            Format::date($last_action_at, false, true),
            formatSessionDuration($how),
            $num_op,
            '<span class="text_bold">' . (isset($mods_names[$last_module]) ? $mods_names[$last_module] : $last_module) . '</span> [' . $last_op . ']',
        ]);
    }
    ...
    cout(
        '<div>'
        . '<span class="text_bold">' . $lang->def('_USER_TOTAL_TIME') . ' : </span>' . formatSessionDuration($total_seconds)
        . ' &nbsp; <a href="index.php?modname=statistic&amp;op=registerUserDetailsExport&amp;id_course=' . (int) $idCourse . '&amp;id=' . (int) $idst_user . '" class="ico-wt-sprite subs_xls" title="' . Lang::t('_EXPORT_XLS', 'standard') . '">'
        . '<span>' . Lang::t('_EXPORT_XLS', 'standard') . '</span></a>'
        . getTable($tb, '_USERS_LIST_DETAILS_CAPTION', 'stats_register_user_details', $footer_html)
        . getBackUi('index.php?modname=statistic&amp;op=registerCourseUsers&amp;id_course=' . (int) $idCourse, $lang->def('_BACK'))
        . '</div>', 'content');
}

/**
 * Righe sessioni (HTML <tr>) per l'export di un utente in un corso, condivisa
 * fra l'export singolo e l'export multiplo del Registro presenze.
 */
function registerBuildSessionRows($idCourse, $idUser, &$session_count, &$total_seconds)
{
    $session_count = 0;
    $total_seconds = 0;
    $rows_html = '';
...
/**
 * Export Excel di un singolo utente, intestato con il suo nome.
 */
function registerUserDetailsExport()
{
    checkPerm('view');
    require_once _base_ . '/lib/lib.download.php';
    ...
    $output = '<table border="1">'
        . '<tr><th colspan="4">' . htmlspecialchars($fullname !== '' ? $fullname : $username) . ' (' . htmlspecialchars($username) . ')</th></tr>'
        . '<tr>'
        . '<th>' . $lang->def('_SESSION_STARTED') . '</th>'
        . '<th>' . $lang->def('_LAST_ACTION_AT') . '</th>'
        . '<th>' . $lang->def('_HOW_MUCH_TIME') . '</th>'
    ...
    $output .= registerBuildSessionRows($idCourse, $idst_user, $session_count, $total_seconds);
    ...
    sendStrAsFile($output, 'registro_presenze_' . preg_replace('/[^a-zA-Z0-9]/', '_', $username) . '_' . date('Ymd') . '.xls');
    exit();
}

/**
 * Export Excel di piu' utenti selezionati: una sezione per allievo,
 * intestata con il suo nome.
 */
function registerExportSelected()
{
    ...
    foreach ($selected as $idst_user) {
        ...
        $output .= '<tr><th colspan="4">' . htmlspecialchars($fullname !== '' ? $fullname : $username) . ' (' . htmlspecialchars($username) . ')</th></tr>'
            . '<tr>'
            . '<th>' . $lang->def('_SESSION_STARTED') . '</th>'
            . '<th>' . $lang->def('_LAST_ACTION_AT') . '</th>'
            . '<th>' . $lang->def('_HOW_MUCH_TIME') . '</th>'
    ...
    sendStrAsFile($output, 'registro_presenze_' . date('Ymd') . '.xls');
    exit();
}
...
function statisticDispatch($op)
{
    switch ($op) {
        ...
        case 'userdetails_export_all':
            ...
            break;
        case 'registerSelectCourse':
            registerSelectCourse();
            break;
        case 'registerCourseUsers':
            registerCourseUsers();
            break;
        case 'registerUserDetails':
            registerUserDetails();
            break;
        case 'registerUserDetailsExport':
            registerUserDetailsExport();
            break;
        case 'registerExportSelected':
