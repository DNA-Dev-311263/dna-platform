    ...
    foreach ($user_coursepath as $id_path => $path_info) {
        ...
        foreach ($coursepath_courses[$id_path] as $id_course => $course_info) {
            ...
            if ($course_info['status'] != _CUS_END && $unlock) {
                $query_control_info = 'SELECT c.idCourse, c.course_type, c.idCategory, c.code, c.name, c.description, c.difficult, c.status, c.course_edition, '
        ...
        foreach ($coursepath_courses[$id_path] as $id_course => $course_info) {
            ...
            $query_control_info = 'SELECT c.idCourse, c.course_type, c.idCategory, c.code, c.name, c.description, c.difficult, c.status, c.course_edition, '
                                    ...
                                    . ' JOIN %lms_courseuser AS cu ON (c.idCourse = cu.idCourse) '
									. ' WHERE c.idCourse = ' . $id_course
									. ' AND cu.idUser = ' . (int) Docebo::user()->getIdst();
            ...
            echo '<tr>'
                    ...
                    . '<td data-reason="'.$course_access["reason"].'">' . ($course_access['can'] ? '<a class="std_link" href="index.php?modname=course&amp;op=aula&amp;idCourse=' . $course_info['idCourse'] . '">' : '') . $course_info['name'] . ($course_access['can'] ? '</a>' : '') . '</td>'
                    . '</tr>';
        }
        ...
        echo	"<script type='text/javascript'> 
					var compl = '".$path_info['percentage']."';
					if (compl != '100') {
						expandCourses('".$id_path."', '".$type."');
					}
				</script>";
    }
