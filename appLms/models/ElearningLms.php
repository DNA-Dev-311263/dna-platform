...
class ElearningLms extends Model
{
    ...
    protected $course_man = false; //ABR
    ...
    public function findAll($conditions, $params)
    {
        ...
        if (!empty($courses)) {
			...
			// ABR: inserisco se il corso ha nuovi contentuti
			$previous_enter = Docebo::user()->getPreviousEnter();
			$new_contents = $this->course_man->checkNewContent($courses, $previous_enter);

			foreach ($new_contents as $id => $new) {
				$result[$id]['new_content'] = $new;
			}
        }
