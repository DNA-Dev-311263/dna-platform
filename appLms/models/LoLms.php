...
class LoLms extends Model
{
    ...
    public function getRootFolderStatus()
    {
        $rootFolder = $this->tdb->getFolderById(0);
        return $this->treeView->getFolderStatus($rootFolder);
    }

    public function getFolderStatusById($folderId)
    {
        $folder = $this->tdb->getFolderById($folderId);
        return $this->treeView->getFolderStatus($folder);
    }
}
