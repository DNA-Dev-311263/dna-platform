...
class DateManager
{
	...
	public function getClassroomForDropdown($location_virtual = false)
	{
		...
		$query =	"SELECT idClassroom, location, name"
					...
					." WHERE loc.location_virtual = ".(int)$location_virtual;
		...
		if (Docebo::user()->getUserLevelId() !== ADMIN_GROUP_GODADMIN) {
			...
			if (!empty($arr_locations)) {
				$query .= " AND loc.location_id IN (".implode(",", $arr_locations).") ";
			} else {
				return $res;
			}
		}
	...
	public function insDate($id_course, $code, $name, $description, $medium_time, $max_par, $price, $overbooking, $status, $test_type, $sub_start_date, $sub_end_date, $unsubscribe_date_limit, $internal_note = '')
	{
		//Mod. ABR
		$query =	"INSERT INTO %lms_course_date "
					." (`id_course`, `code`, `name`, `description`, `medium_time`, "
					." `max_par`, `price`, `overbooking`, `test_type`, `status`, "
					." `sub_start_date`, `sub_end_date`, `unsubscribe_date_limit`, `calendarId`, `internal_note`) "
					...
					." '".$unsubscribe_date_limit."', '" . CalendarManager::generateUniqueCalendarId() . "', '".$internal_note."')";
    ...
    public function getCourseDate($id_course, $all = true, $ini = 0, $num_element = 0)
    {
        ...
        $query = 'SELECT dt.*, MIN(dy.date_begin) AS date_begin, MAX(dy.date_end) AS date_end, COUNT(dy.id_day) as num_day, COUNT(DISTINCT du.id_user) as user_subscribed, GROUP_CONCAT(du.id_user) as usersids'
            . ' FROM %lms_course_date as dt'
            . ' LEFT OUTER JOIN %lms_course_date_day  as dy ON (dy.id_date = dt.id_date AND deleted = 0)'
    ...
    public function getClassromByID($id_classroom)
    {
     ...
     public function getClassromInfoByID($id_classroom)
    {
		//Ins. ABR
		$res = array();
        $query =    "SELECT c.name, l.location, c.description, c.room, c.street, c.city"
                    ." FROM %lms_classroom c"
                    ." 		JOIN %lms_class_location l ON c.location_id = l.location_id"
                    ." WHERE c.idClassroom = ".(int)$id_classroom;
    ...
    public function getLocations($location_id = false)
    {
		//Ins. ABR
		
		$whrExp = (is_numeric($location_id) ? ' WHERE location_id = '.(int)$location_id : '');
		$res = array();
        ...
        while($row = sql_fetch_assoc($result))
			$res[$row['location_id']] = $row['location'];
	...
	{
		//Mod. ABR
		$query =	"SELECT dt.*, MIN(dy.date_begin) AS date_begin, MAX(dy.date_end) AS date_end, COUNT(dy.id_day) as num_day, COUNT(DISTINCT du.id_user) as user_subscribed, MIN(location_id) as location_id"
					." FROM %lms_course_date as dt"
					." JOIN %lms_course_date_day as dy ON dy.id_date = dt.id_date"
					." LEFT JOIN %lms_course_date_user as du ON du.id_date = dt.id_date"
					." LEFT JOIN %lms_classroom as cl ON dy.classroom = cl.idClassroom"
					." WHERE dt.id_date = " . (int)$id_date . " AND dy.deleted = 0"
					." GROUP BY dt.id_date"
					." ORDER BY dy.date_begin";
	...
	/** ABR **
	 * Recupera le informazioni di tutte le date passate in argomento
	 * 
	 */
	public function getDatesInfo($arr_id_date, $status = false, $desc = false)
	{
		$whr_exp = "";
		$res = array();
		
		if (is_array($status)) 
			$whr_exp = " AND status IN ('". implode("','", $status) ."')";	
					
		elseif ($status !== false)
			$whr_exp = " AND status = ".(int)$status;
		...
		$query =	"SELECT dt.*, MIN(dy.date_begin) AS date_begin, MAX(dy.date_end) AS date_end, COUNT(dy.id_day) as num_day, COUNT(DISTINCT du.id_user) as user_subscribed, MIN(location_id) as location_id"
					...
					." LEFT JOIN %lms_course_date_user as du ON du.id_date = dt.id_date"
					." LEFT JOIN %lms_classroom as cl ON dy.classroom = cl.idClassroom"
					." WHERE dt.id_date IN ('".implode("','", $arr_id_date)."') AND dy.deleted = 0" . $whr_exp 
					." GROUP BY dt.id_date"
					." ORDER BY dy.date_begin ". ($desc ? 'DESC' : 'ASC');
    ...
    public function getAllDateDay($id_date, $pattern = [])
    {
        ...
        $query = 'SELECT *, DATE_FORMAT(date_begin, "%d-%m-%Y") as date'
            . ' FROM %lms_course_date_day'
            . ' WHERE deleted = 0 AND id_date = ' . $id_date
            . ' ORDER BY date_begin';
	...
	{
		//Mod. ABR dic-21
		$query =	"SELECT classroom, date_begin, date_end, pause_begin, pause_end, id_day, id"
					." FROM %lms_course_date_day "
					." WHERE deleted = 0 AND id_date = ".$id_date;
		...
		while(list($classroom, $date_begin, $date_end, $pause_begin, $pause_end, $id_day, $id) = sql_fetch_row($result))
		{
    ...
    public function upDate($id_date, $code, $name, $description, $medium_time, $max_par, $price, $overbooking, $status, $test_type, $sub_start_date, $sub_end_date, $unsubscribe_date_limit, $internal_note = '')
    {
		//Mod. ABR
    ...
    public function getDateDayDateDetails($id_date)
    {
		...
		//>> ABR: Inserite anche le informazioni sulla pausa e id_day
 	...
 	public function getUserPresenceForDate($id_date)
	{
        ...
        if (Docebo::user()->getUserLevelId() == ADMIN_GROUP_ADMIN) {
			...
			//ABR: Se l'utente non è amministratore, controllo se è un docente
			$teachers = $this->getDateTeachers($id_date, false);
			
			if(!array_key_exists(Docebo::user()->getIdSt(), $teachers)){
				//Se l'utente non è nemmeno un docente del corso, invalido la query di estrazione
				$query .= " AND 0";
			}
        }
	...
	public function getDateAlreadyFull($id_course)
    {
		//ABR: restituisce gli id delle classroom senza posti
		// 	Metto in join la tabella degli utenti perché forma, 
		//  al momento, non elimina le iscrizioni con l'eliminazione degli utenti
		
		$query =	"SELECT dt.id_date "
					." FROM %lms_course_date as dt"
					." 		JOIN %lms_course_date_user as du ON du.id_date = dt.id_date"
					." 		JOIN %adm_user as u ON du.id_user = u.idst"
					." WHERE dt.max_par > 0 AND dt.id_course = ".(int)$id_course
					." GROUP BY dt.id_date, dt.max_par "
					." HAVING COUNT(DISTINCT du.id_user) >= dt.max_par "
					." ORDER BY dt.id_date";
    ...
    public function getUserDateForCourse($id_user, $id_course, $only_open = false)
    {
		//Mod. ABR: aggiungo filtro sullo stato della data
        $res = [];
        $whrExp = ""; 		//ABR
        ...
        if ($id_user !== $aclManager->getAnonymousId()) {
			...
			$whrExp = ($only_open ? ' AND date_complete = ' . _DATE_STATUS_ACTIVE : ''); //ABR
			
            $query = 'SELECT id_date  FROM %lms_course_date_user   WHERE id_user = ' . (int)$id_user
                . ' AND id_date IN (  SELECT id_date  FROM %lms_course_date   WHERE id_course = ' . (int)$id_course . $whrExp . ')';
    ...
    public function getCourseEdition($id_course, $start_index = false, $results = false, $sort = false, $dir = false, $ed_arr = false)
    {
        ...
        $query = 'SELECT dt.id_date, dt.code, dt.name, dt.status, MIN(dy.date_begin) AS date_begin, MAX(dy.date_end) AS date_end, COUNT(dy.id_day) as num_day, COUNT(DISTINCT u.idst) as user_subscribed, dt.unsubscribe_date_limit, dt.internal_note, dt.sub_start_date, dt.sub_end_date'
            . ' FROM %lms_course_date as dt'
		...
		//ABR: aggiunti campi di risultato
        while (list($id_date, $code, $name, $status, $date_begin, $date_end, $num_day, $user_subscribed, $unsubscribe_date_limit, $internal_note) = sql_fetch_row($result)) {
            if (Docebo::user()->getUserLevelId() != ADMIN_GROUP_GODADMIN) {
                require_once _base_ . '/lib/lib.preference.php';
                $adminManager = new AdminPreference();
                $query = 'SELECT COUNT(*)'
                    . ' FROM %lms_course_date_user AS du'
                    . ' JOIN %adm_user AS u ON u.idst = du.id_user'
                    . " WHERE du.id_date = '" . $id_date . "'"
                    . ' AND ' . $adminManager->getAdminUsersQuery(Docebo::user()->getIdSt(), 'du.id_user');
            ...
            $res[] = [
                ...
                'internal_note' => $internal_note, //ABR
    ...
    /** ABR **
	 * Recupera i docenti dell'edizione.
	 * Il livello devo prenderlo dalla courseuser che, al momento, non ha la chiave dell'edizione valorizzata.
	 * Infatti un utente non può avere, per uno stesso corso, il ruolo di amministratore in un edizione e di docente in un altra edizione.
     * Il ruolo è unico per corso. 
     */
 	public function getDateTeachers($id_date, $in_string = true)
	{

		
		$res = false;
		
		$query = 	"SELECT DISTINCT u.firstname AS t_firstname, u.lastname AS t_lastname, u.email AS t_email, u.idst "
					." FROM %adm_user u"
					." JOIN %lms_course_date_user cdu ON u.idst = cdu.id_user"
					." JOIN %lms_course_date cd ON cdu.id_date = cd.id_date"
					." JOIN %lms_courseuser cu ON (cd.id_course = cu.idCourse AND cdu.id_user = cu.idUser)"
					." WHERE cu.level = 6 AND cd.id_date = '".$id_date."'";
		...
		if($result){
			...
			if(!$in_string){
				...
				//passo una sola stringa con i docenti concatenati
				while(list($firstname, $lastname) = sql_fetch_row($result))
    ...
    /** ABR **
     * restituisce i giorni per ogni edizione 
     */
	public function getDateDayForDates($id_dates, $only_valid = false)
	{
		$whr = $only_valid ? " AND dy.deleted = 0 " : "";
		
		$query =	"SELECT dy.*, cl.name AS class_name, cl.room AS class_room, lc.location "
					." FROM %lms_course_date_day dy "
					."		LEFT JOIN %lms_classroom cl ON dy.classroom = cl.idClassroom "
					."		LEFT JOIN %lms_class_location lc ON cl.location_id = lc.location_id "
					." WHERE id_date IN (".implode(',', $id_dates).") " . $whr
					." ORDER BY date_begin";
	...
	public function getDaysAndLocations($id_date, $format_date_name = 'month_name_short') {
		//>> ABR: restituisce un array con le date dell'edizione e le aule.
		
		// Recupero i giorni con orari
		$days = $this->getDateDayForControl($id_date);
		
		// Recupero info classe
		foreach ($days as $k => $v){
			
			$info_loc = $this->getClassromInfoByID($v['classroom']);
			
			// la classe per ogni giorno
			$days[$k]['class'] =  $info_loc['name'];
			
			// la room per ogni giorno (eventuale differenziazione, modalità virtuale)
			$days[$k]['room'] =  $info_loc['room'];
				
			// la sede per ogni giorno
			$days[$k]['location'] =  $info_loc['location'];
			
			// id del giorno
			$days[$k]['id_day'] = $v['id_day'];
			
			// preparo una stringa formattata con le informazioni
			$days[$k]['format_info_day'] = Format::datetimeToString($k, $format_date_name)
									   .($v['b_hours'] !== '00' ? ', '.$v['b_hours'].':'.$v['b_minutes'] : '')
									   .($v['e_hours'] !== '00' ? ' - '.$v['e_hours'].':'.$v['e_minutes'] : '');
		}
		
		// Ordino le chiavi (date)
		ksort($days);
	...
	/** ABR **
	 * Restituisce gli utenti iscritti a una specifica edizione classroom
	 */
	public function getDateUsers($id_date, $only_open = false) {

		$whrExp = ($only_open ? 'AND date_complete = ' . _DATE_STATUS_ACTIVE : '');

		$query =	"SELECT id_user"
					." FROM %lms_course_date_user"
					." WHERE id_date = ".$id_date.' '.$whrExp;
	...
	public function getUserDays($id_user, $only_status_active = true) {
		//>> ABR: restituisce i giorni impegnati in un corso di un determinato utente
		
		$whrExp = ($only_status_active ? 'AND dt.status = '._DATE_STATUS_ACTIVE : '');

		$query =	"SELECT dy.id_day, date(dy.date_begin) AS day, dt.id_date, dt.id_course, udt.id_user "
					." FROM %lms_course_date_user AS udt "
					."		JOIN %lms_course_date AS dt ON udt.id_date = dt.id_date "
					."		JOIN %lms_course_date_day AS dy ON dt.id_date = dy.id_date "
					." WHERE udt.id_user = ". (int)$id_user. " ".$whrExp;

		$result = sql_query($query);
		$res = array();

		while(list($id_day, $day, $id_date, $id_course, $id_user) = sql_fetch_row($result))
			$res[$day] = array('id_day'=>$id_day, 'day'=>$day, 'id_date'=>$id_date, 'id_course'=>$id_course, 'id_user'=>$id_user);

		return $res;
	}
	
	
	public function countSeatByCourse($id_course) {
		//>> ABR: conta i posti disponibili delle edizioni confermate per uno specifico corso
		
		$count = 0;
		$query =	"SELECT id_date, max_par "
					."FROM %lms_course_date AS dt "
					."WHERE id_course = ". (int)$id_course. " AND status = " ._DATE_STATUS_ACTIVE;
		
		//Lancio la query			
		$date = sql_query($query);
		
		//Calcolo i posti disponibili
		if($date){
			while($row = sql_fetch_assoc($date)){
				
				$num_subs = count($this->getDateUsers($row['id_date']));
				$num_seat = $row['max_par'] - $num_subs;
				
				if ($num_seat > 0) $count += $num_seat;
			}	
		}
	...
	public function getDateLocations($id_date) {
		//Ins. ABR, restituisce le sedi per una specifica data
		
		$res = array();
		
		$query = "SELECT DISTINCT lc.location_id, lc.location "
				." FROM %lms_course_date_day dy "
				." 	JOIN %lms_classroom cl ON dy.classroom = cl.idClassroom "
				."	JOIN %lms_class_location lc ON cl.location_id = lc.location_id "
				." WHERE dy.id_date = ".(int)$id_date;
	...
	public function getDateInPeriod($date_begin, $date_end, $id_course = array()) {
		//>> ABR: Restituisce tutte le date in partenza nel periodo specificato
	...
	public function checkSelfUnsubsDate($id_date) {
		//>> ABR: Controlla se le edizioni classroom passate in argomento consentono la disiscrizione e sono entro la data massima.
		//>> L'argomento $id_date può essere un singolo valore o un array.
		//>> Restituisce un array con id_date in chiave.
		...
		$fl = 'unsubscribe_date_limit';
		$tzero = '0000-00-00 00:00:00';
		$now = date('Y-m-d H:i:s');
		$res = array();

		$array_date = (!is_array($id_date) ? array($id_date) : $id_date);
			
		foreach ($array_date as $id_date) {
			//Reset valori
			$m = -1;
			$val = 0;
			
			//Trovo l'edizione
			$edition = $this->getDateInfo($id_date);
			
			
				
			if($edition) {
				
				//Recupero tipo disiscrizione corso
				$query = "SELECT auto_unsubscribe, unsubscribe_date_limit FROM %lms_course WHERE idCourse = ".$edition['id_course'];
				list($course['auto_unsubscribe'], $course['unsubscribe_date_limit']) = sql_fetch_row(sql_query($query));

				//Controlli
				
				if ($course['auto_unsubscribe']!= 0) {
					//self unsubscribe possibile per il corso
					
					if ((!$course[$fl] || $now <= $course[$fl]) && ($edition[$fl] == $tzero || $now <= $edition[$fl])) {
						//self unsubscribing ammessa
						$m = 1;
					}
					
					//Ricavo valore di controllo
					$val = $course['auto_unsubscribe'] * $m;
				}	
			}
			//Passo valore all'array di ritorno
			$res[$id_date] = $val;
		}
	...
	/* ABR */
	public function isCatCourseMailSuspend($id_category) {
		//>> Restituisce se la categoria corso passata in argomento ha l'invio mail sospeso in base al setting di sistema (gestione provvisoria per FBA Allianz)
		
		$res = false;
		
		// Recupero i nomi delle categorie corso con invio mail sospeso (es. "/Comportamentale,/Office,/IT")
		$cat_string = FormaLms\lib\Get::sett('cat_course_mail_suspend', '');
			
		// Preparo array
		$categories = explode(",", $cat_string);
		
		// Query per recupero path cateogira
		$query = "SELECT path FROM %lms_category WHERE idCategory = ".(int)$id_category;
		
		$result = sql_query($query);
		
		// Verifico se il path contiene un nome categoria presente nel setting
		if ($result) {
			list($path) = sql_fetch_row($result);
			
			foreach($categories as $cat) {
				if (stripos($path, $cat) !== false) {
					$res = true;
					break;
				}	
			}	
