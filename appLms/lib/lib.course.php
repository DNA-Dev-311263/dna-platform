...
class Man_Course
{
	 ...
	 * 
	 * @return array	return some info about the course [code, name, description, status, difficult, subscribe_method, max_num_subscribe, category and label]
	 */
	function getCourseMoreInfo($id_course) {
		
		$language = Lang::lang_code();
		
		$query = "SELECT co.*, ca.path AS cat_path, ca.description AS cat_desc, l.title AS label_title, l.description AS label_desc"
					." FROM %lms_course co"
					." 	LEFT JOIN %lms_category ca ON co.idCategory = ca.idCategory"
					." 	LEFT JOIN %lms_label_course lc ON co.idCourse = lc.id_course"
					." 	LEFT JOIN %lms_label l ON lc.id_common_label = l.id_common_label"
					." WHERE co.idCourse = '".$id_course."' AND (l.lang_code IS NULL OR l.lang_code = '".$language."')";
	...
	/**  
	 * --- ABR ---
	 * @param int 	$id_obj			id del corso o dell'edizione (default corso)
	 * @param str	$filter			espressione per aggiungere un filtro all'estrazione.
	 * @param str 	$area_code		codice dell'area del campo (es. COURSE, COURSE_EDITION...)
	 * 
	 * @return array	restituisce le informazioni inserite nei campi custom del corso
	 */
	function getCourseCustomFields($id_obj, $hideV0 = false, $filter = false, $area_code = "COURSE")
	{
		$cfields = array();
		$language = Lang::lang_code();
		$whrExp = (!$hideV0 ? "" : " AND (cf.code NOT LIKE '!V0%') ");
		$whrExp .= (!$filter ? "" : " AND (".$filter.") ");
		
		
		$query = "SELECT cf.area_code AS cf_area_code, cf.code AS cf_code, cfl.translation AS cf_name, cfe.obj_entry AS cf_value, cfsl.translation AS cf_combo_value"
					." FROM %adm_customfield cf"
					." 	JOIN %adm_customfield_entry cfe ON cf.id_field = cfe.id_field"
					." 	JOIN %adm_customfield_lang cfl ON cfe.id_field = cfl.id_field"
					." 	LEFT JOIN %adm_customfield_son cfs ON cf.id_field = cfs.id_field AND cfe.obj_entry = cfs.id_field_son"
					." 	LEFT JOIN %adm_customfield_son_lang cfsl ON cfs.id_field_son = cfsl.id_field_son  AND cfl.lang_code = cfsl.lang_code"
					."	WHERE cf.area_code = '".$area_code."' AND cfe.id_obj = '".$id_obj."' AND cfl.lang_code = '".$language."' ".$whrExp;
					
					
		$re_cfields = sql_query($query);
		
		
		//array di ritorno con codice campo per indice
		while($cfield = sql_fetch_array($re_cfields)) {
			$cfields[$cfield['cf_code']] = $cfield;
		}
    ...
    public function getAllLabels()
    {
        ...
        $query =  " SELECT id_common_label, title, description, file_name, sequence "
				. " FROM %lms_label";

        $result = sql_query($query);
        
        while($row = sql_fetch_assoc($result))
			$res[$row['id_common_label']] = $row;
...
VALUES ('" . $idCourse . "', '" . $id_module . "', '" . $id_main . "', '" . $i++ . "', '')");
    ...
    /**
	 * ABR: Restituisce l'ID corso dal codice
	 */
    public function getCourseIdByCode($code)
    {
        $output = false;
        $query = "SELECT idCourse FROM %lms_course WHERE code LIKE '" . $code . "'";
        $res = sql_query($query);
        if ($res && (sql_num_rows($res) > 0)) {
            list($idCourse) = sql_fetch_row($res);
            $output = $idCourse;
	...
	/**
	 * ABR: Restituisce gli stati corso
	 */
	...
	/**
	 * ABR: Restituisce le categorie corso figlie di una categoria (spostata dal model)
	 */
	...
	/**
	 * ABR: Recupera i docenti del corso. Da usare solo per i corsi e-learning a edizione unica.
	 * Per i corsi a edizioni e classroom c'è il metodo nelle libreria dedicata.
	 */
	public function getCourseTeachers($id_course, $in_string = true)
	{
		$res = false;
		
		$query = 	"SELECT DISTINCT u.firstname AS t_firstname, u.lastname AS t_lastname, u.email AS t_email, cu.level, u.idst "
					." FROM %adm_user u"
					." JOIN %lms_courseuser cu ON u.idst = cu.idUser "
					." JOIN %lms_course c ON cu.idCourse = c.idCourse "
					." WHERE c.course_type = 'elearning' AND c.course_edition = 0 AND cu.level IN (6,4) AND cu.idCourse = '".$id_course."'";
	...
	/**
	 * ABR: Controlla se i corsi passati in argomento consentono la disiscrizione e sono entro la data massima
	 */
	public function checkSelfUnsubsCourse($id_course) {
		//>> L'argomento $id_Course può essere un singolo valore o un array.
		//>> Restituisce un array con id_course in chiave.
		...
		$array_course = (!is_array($id_course) ? array($id_course) : $id_course);
	...
	/**
	 * ABR: Restituisce i corsi del catalogo assegnati all'organizzazione 
	 */
	public function getCourseCatalogOrg($id_org, $status = false, $subscribe_method = false) {
		...
		if(is_array($subscribe_method)) {
			$whrExp .= " AND cor.subscribe_method IN (" . implode(',', $subscribe_method) . ") ";
		}
		...
		$query = "SELECT cte.idCatalogue, cor.* "
				...
				."WHERE ogc.idOrg = " . (int)$id_org . " AND cte.type_of_entry = 'course' " . $whrExp
				."ORDER BY cor.code";
	...
	/**
	 * ABR: Restituisce l'ultimo lo inserito del corso
	 * Possibile trovare l'ultimo in base a tipologia
	 */
	public function getLastLo($id_course, $objectType = false)
	{
		// Preparo query
		$query = "SELECT * FROM %lms_organization WHERE idCourse = " . (int)$id_course;
		$query.= $objectType ? " AND objectType = '".$objectType."'" : "";
		$query.= " ORDER BY dateInsert DESC LIMIT 1";
	...
	/**
	 * ABR: Trova se il corso ha nuovi contenuti
	 * Se il momento corrente è tra la data di creazione dell'ultimo contenuto e l'istante di permanenza finale della segnalazione,
	 * oppure se $date_check è precedente alla data di creazione dell'ultimo contenuto
	 * la funzione restituisce un array di valori booleani con id_course come chiave
	 * @param $course_ids array, chiavi corso
	 * @param $date_check string, data di controllo (es. ultimo accesso utente).
	 */
	public function checkNewContent($course_ids, $date_check = false) {
		...
		// Recupero info corso
		$query = "SELECT * FROM %lms_course WHERE idCourse IN (" .implode(",", $course_ids). ")";
		$courses = sql_query($query);
				
		while($info = sql_fetch_assoc($courses))
		{
			$result = false;
			$id_course = $info['idCourse'];
			
			// Recupero ultimo LO
			$lo = $this->getLastLo($id_course);
			
			// Calcolo se esistono nuovi contenuti per ogni corso
			if ($lo) {
				$time_insert = strtotime($lo['dateInsert']);
				$last_permanence = $time_insert + ($info['new_content_indicator_days'] * 24 * 3600);
				
				if ( $last_permanence > $time_insert) {
					$result = ( $now <= $last_permanence || ($date_check && strtotime($date_check) < $time_insert) );
				}
					
			}
			
			//Preparo array di ritorno
			$res[ $id_course ] = (bool) $result;
		}
...
class Man_CourseUser
{
	 ...
	 * Mod. ABR
	 */
	 function &getUserCourses($id_user, $id_category = false, $course_status = false, $user_status = false, $sort_field = false) {
		//ABR: modificato argomento input user_status (accetta array) e aggiunto campo di ordinamento

		$courses = array();
		
		// Where
		if ($user_status && !is_array($user_status))
		...
		switch ($sort_field) {
		  case "code":
			...
			break;
		  case "date_inscr":
			$sort_field = "u.date_inscr DESC";
			break;
		  case "date_complete":
			$sort_field = "u.date_complete DESC";
			break;
		  default:
			$sort_field = "c.name";
		} 	 
		...
		SELECT c.idCourse, c.idCategory, c.code, c.name, c.description,
			c.date_begin, c.date_end, c.hour_begin, c.hour_end, c.valid_time, c.status AS course_status, 
			c.course_type, c.course_edition, c.unsubscribe_date_limit, c.userStatusOp,
			u.waiting, u.level, u.status as user_status, u.date_inscr, u.date_first_access, u.date_complete, u.idUser 
		...
		if($user_status !== false) 		$query_courses .= " AND u.status IN ('".implode("','", $user_status)."') ";
		
		$query_courses .= "ORDER BY ".$sort_field;
    ...
    /**
     *  return true if the user has completed all courses in array courseIdsArr. Mod. ABR
     */
    public function hasCompletedCourses($id_user, $courseIdsArr)
    {
       return  $this->countCompletedCourses($id_user, $courseIdsArr) == count($courseIdsArr);
    }
    
    
    /**
     *  ABR: return number of course completed.
     */
    public function countCompletedCourses($id_user, $courseIdsArr)
    {
