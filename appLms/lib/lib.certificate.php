...
class Certificate
{
   ...
   /**
     * ABR: Elabora eventuale funzioni IIF inserite nel template
     */
    protected function evalTemplFunction($cert_template) {
		
		$templ = $cert_template;
		
		try {
			if (strpos($templ, "{") !== false && strpos($templ, "}") !== false) {
				
				$pattern1 = '#\{(.*?)\}#';
				$templ = preg_replace_callback($pattern1, function($match1) {
						
					$exp = $match1[1];
					
					// Sostituisco gli operatori
					$exp = str_replace(['=', '<>', 'AND', 'OR'], ['==', '!=', '&&', '||'], $exp);
					

					// Gestione funzione tag IIF
					if (strpos($exp, "IIF(") !== false) {

						// Sostituisco IIF (non gestisce annidamenti)
						$pattern2 = '#IIF\((.*?)\)#';
						$exp = preg_replace_callback($pattern2, function($match2) {
							
							$args = explode(',',$match2[1]);
							
							// Espressione test della IF
							$test_string = "return (".$args[0].");";
							$test = eval($test_string);
							
							// Levo apici al se_vero e se_falso
							$args[1] = str_replace("'", "", $args[1]);
							$args[2] = str_replace("'", "", $args[2]);
							
							// IF e restituisco il risultato
    ...
    public function send_certificate($id_certificate, $id_user, $id_course, $array_substituton = false, $download = true, $from_multi = false, $id_association = 0)
    {
        ...
        if ($array_substituton !== false) {
            ...
            $cert_structure = $this->evalTemplFunction( $cert_structure ); //ABR: elaboro eventuali funzioni del template
        }
        $cert_structure = fillSiteBaseUrlTag($cert_structure);
