...
class CoursePath_Manager
{
    ...
    /**
     * ABR
     * Restituisce l'id del percorso dal codice
     */
    public function getIdByCode($coursepath_code)
    {
        $q = "SELECT id_path"
			. " FROM " . $this->_getPathTable()
			. " WHERE path_code ='" . $coursepath_code ."'";
	...
	/**
	 * ABR: Ricalcola il numero di corsi completati dall'utente nella tabella del percorso
	 */
	public function resetCourseCompleted($id_path, $id_user) {
		
		$man_courseuser = new Man_CourseUser(DbConn::getInstance());
		$courseIdsFromPath = $this->getPathCourses(intval($id_path));
		$count = $man_courseuser->countCompletedCourses($id_user, $courseIdsFromPath );
		
		$query = "UPDATE ". $this->_getPathUserTable() ." SET course_completed = ".$count." WHERE id_path  = ". (int) $id_path ." AND idUser = ". (int) $id_user;
        return sql_query($query);
	}
    
    
	/**
	 * ABR: Restituisce i percorsi del catalogo assegnati all'organizzazione 
	 */
	public function getCoursepathCatalogOrg($id_org, $subscribe_method) {
		...
		$query = "SELECT cte.idCatalogue, cor.* "
				...
				."	JOIN %lms_coursepath cor ON cte.idEntry = cor.id_path "
				...
				."WHERE ogc.idOrg = " . (int)$id_org . " AND cte.type_of_entry = 'coursepath' " . $whrExp
				."ORDER BY cor.path_code";
