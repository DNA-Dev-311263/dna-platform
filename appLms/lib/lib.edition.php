...
{
    public $edition_table;
    public $edition_user;
    public $course_table;
    public $courseuser_table;
    public $user_table;
    ...
    public function getUserEdition($id_user, $status = false, $id_course = false)
    {
		//>> ABR: aggiunti argomenti $status e $id_course
		
		$query =	"SELECT eu.id_edition"
					." FROM ".$this->edition_user." eu"
					."		JOIN ".$this->edition_table." e ON eu.id_edition = e.id_edition"
					." WHERE id_user = '".$id_user."'" 
					. 		($status	=== false ? '' : ' AND e.status = '.(int)$status)
					. 		($id_course	=== false ? '' : ' AND e.id_course = '.(int)$id_course);
	...
	public function getEditionTeachers($id_edition, $in_string = true)
	{
		//ABR: recupera i docenti dell'edizione.
		...
		$query = 	"SELECT DISTINCT u.firstname AS t_firstname, u.lastname AS t_lastname, u.email AS t_email, u.idst "
					...
					." JOIN %lms_course_editions_user ceu ON u.idst = ceu.id_user"
					." JOIN %lms_course_editions ce ON ceu.id_edition = ce.id_edition"
					." JOIN %lms_courseuser cu ON (ce.id_course = cu.idCourse AND ceu.id_user = cu.idUser)"
					." WHERE cu.level = 6 AND ce.id_edition = '".$id_edition."'";
