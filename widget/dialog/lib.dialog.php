...
class DialogWidget extends Widget
{
    ...
    public $buttons = []; //deprecated, button are automatically set
    ...
    //ABR: calling delegate and function 
    public $callDelegateEvents = false;
    public $callObjectFunc = false;
    ...
    public function run()
    {
        ...
        if ($this->renderEvent) {
            $params['renderEvent'] = $this->renderEvent;
        }
        if ($this->destroyEvent) {
            $params['destroyEvent'] = $this->destroyEvent;
        }
        ...
        if (!is_array($this->callEvents)) {
            ...
            foreach ($arrEvents as $event => $list) {
                if (is_array($list)) {
                    $callEvents .= 'YAHOO.util.Event.addListener(' . $this->json->encode($list) . ', "' . $event . '", dialogEvent);' . "\n";
                } else {
                    $callEvents .= 'YAHOO.util.Event.addListener(' . $list . ', "' . $event . '", dialogEvent);' . "\n";
                }
            }
        }
        ...
        //ABR: set calling delegate events 
        $callDelegateEvents = '';
        if ( is_array($this->callDelegateEvents) ) {

            $arrEvents = [];
            foreach ($this->callDelegateEvents as $val) {
                    $arrEvents[ $val['event'] ]['caller'] = $val['caller'];
                    $arrEvents[ $val['event'] ]['matchedEl'] = $val['matchedEl'];
            }
            foreach ($arrEvents as $event => $list) {
                    $callDelegateEvents .= 'YAHOO.util.Event.delegate("' . $list['caller'] . '", "' . $event . '", dialogEvent, "'. $list['matchedEl'] .'");' . "\n";
            }
        }
        $params['callDelegateEvents'] = $callDelegateEvents;
        
        
        //ABR: set caller external function
        $callObjectFunc = '';
		if ($this->callObjectFunc) {
			 $callObjectFunc = $this->callObjectFunc.'["'.$this->id.'"] = dialogEvent;' . "\n";
		}
		$params['callObjectFunc'] = $callObjectFunc;
