        ...
        <div class="filterBar__advanced collapse navbar-collapse" id="filter-container">
            <div class="simple_search_box" id="<?php echo $id; ?>_simple_filter_options" style="display: block;">
                ...
                <div class="filterBar__mainsearch navbar-form form-group">
                    ...
                    <select id="courseassn_search_filter_type" name="filter_type" class="selectpicker" data-width=""
                    ...
                    <script>
                        ...
                        $(function(){
                            v = getCookie(this_user+'.my_courseassn.type');
                            if (v != '') {$("#courseassn_search_filter_type").selectpicker('val', v );}
                            v = getCookie(this_user+'.my_courseassn.year');
                            if (v != '') {$("#courseassn_search_filter_year").selectpicker('val', v);}
                            courseassn_search_callback_set()
                        })
                        
                        $('#courseassn_search_filter_cat').selectpicker({
                            countSelectedText: '{0} <?php echo Lang::t('_SELECTED', 'course'); ?>'
                        });

                        v = getCookie(this_user+'.my_courseassn.category');
                        v = ( v !='')? v.split(','): ["0"]
                        $('#courseassn_search_filter_cat').selectpicker('val',v) 
                        
                        var prev = v;
                        $('#courseassn_search_filter_cat').on('changed.bs.select', function (e) {
                            if ($(this).val() == null)  {
