...
class XLSXWriter
{
	...
	public function writeSheetRow($sheet_name, array $row, $row_options=null, $row_formats=null)
	{
		//Mod. ABR
		if (empty($sheet_name))
			return;
		...
		foreach ($row as $v) {
			if (empty($format)) {
				...
				$number_format = self::numberFormatStandardized(isset($format[$c]) ? $format[$c] : $format[0]);
				$number_format_type = self::determineNumberFormatType($number_format);
			}
			$cell_style_idx = (empty($style) && empty($format)) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle( $number_format, json_encode(isset($style[0]) ? $style[$c] : $style) );
