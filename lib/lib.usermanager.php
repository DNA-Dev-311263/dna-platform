...
*/
    ...
    public function processRegistrationCode(&$acl_man, &$uma, $iduser, $reg_code, $registration_code_type)
    {
        ...
        //ABR
        if ( !empty( $array_coursepath ) ) {
			require_once _lms_ . '/lib/lib.coursepath.php';
			
			$path_man = new CoursePath_Manager();
			
			$path_courses = $path_man->getAllCourses( $array_coursepath );
			
			foreach ($array_coursepath as $id_path) {
				$path_man->subscribeUserToCoursePath($id_path, [$iduser]);
			}
			
			// Eseguo il merge per ottenere un'unica lista di corsi a cui iscrivere l'utente
			$array_course = !isset($array_course) ? $path_courses : $array_course + $path_courses;
		}
    ...
    public function _opt_in($options, $platform, $opt_link)
    {
       ...
       // FIX BUG 399, MOD. ABR 
        ...
        //ABR 
        $code_manager = new CodeManager(); 
        $folders = $code_manager->getOrgAssociateWithCode($reg_code);   
        $folders = $uma->getInfoFolders($folders); 
        $idOrg = reset( array_keys($folders['id_org']) ); 
         
        $dynamicUrl = getCurrentDomain($idOrg) ?: FormaLms\lib\Get::site_url();  
    ...
    private function _first_of_all($options, $platform, $errors = [])
    {
        ...
        if ($options['use_advanced_form'] == 'off') {
			...
			//ABR 1 != 1, sospesa proposta dei gruppi in fase di iscrizione
            if (!empty($groups  && 1 != 1 )) {
                $out .= '<div class="homepage__row homepage__row--gray">'
                    . $lang->def('_SUBSCRIBE')
                    . '</div>'
