...
class Layout
{
    ...
    public static function courseMenu()
    {
        ...
        if (!Docebo::user()->isAnonymous() && $sessionIdCourse) {
            ...
            $main_menu_id = FormaLms\lib\Get::req('id_main_sel', DOTY_INT, 0); 			//es. id student area
            $module_menu_id = FormaLms\lib\Get::req('id_module_sel', DOTY_INT, 0);		//es. id contenuti didattici (menu under)
