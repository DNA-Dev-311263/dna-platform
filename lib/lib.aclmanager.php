...
define("ACL_INFO_PREVIOUSENTER", 20); //ABR
...
class DoceboACLManager
{
                                 ...
                                 $facebook_id = false, $twitter_id = false, $linkedin_id = false, $google_id = false)
    {
        ...
        if (FormaLms\lib\Get::sett('pass_max_time_valid') != 0) {
            $pwd_expire_at = date('Y-m-d H:i:s', time() + FormaLms\lib\Get::sett('pass_max_time_valid') * 24 * 3600);
        }
                               ...
                               $facebook_id = false, $twitter_id = false, $linkedin_id = false, $google_id = false)
    {
        ...
        if ($pass !== false) {
            ...
            if (FormaLms\lib\Get::sett('pass_max_time_valid') != 0) {
                $new_userdata[ACL_INFO_PWD_EXPIRE_AT] = date('Y-m-d H:i:s', time() + FormaLms\lib\Get::sett('pass_max_time_valid') * 24 * 3600);
            }
        }
        if ($email !== false) {
        ...
        if (array_key_exists(ACL_INFO_PASS, $new_userdata) && !FormaLms\lib\Get::cfg('demo_mode')) {
        ...
        //ABR (registro penultimo accesso)
        if ($lastenter !== FALSE)
			$query .= " previous_enter = CASE WHEN '".$lastenter."' > lastenter THEN lastenter ELSE previous_enter END, ";	
    ...
    public function deleteUser($idst)
    {
        ...
        if ($control) {
            ...
            $query1 = 'DELETE FROM %lms_courseuser where idUser = ' . (int)$idst;
    ...
    public function getUser($idst, $userid)
    {
        ...
        $query = 'SELECT idst, userid, firstname, lastname, pass, email, avatar, signature,'
            ...
            . ', force_change, facebook_id, twitter_id, linkedin_id, google_id, privacy_policy, previous_enter '
            . ' FROM ' . $this->_getTableUser();
    ...
    public function getGroupUMembersNumber($idst, $filter = '')
    {
        ...
        if (!$this->include_suspended) {
            if (FormaLms\lib\Get::req('modname', DOTY_ALPHANUM, '') !== 'directory') {
    ...
    public function _getSearchUsersBaseQuery($internal_fields, $extra_fields = false, $idst_filter = false)
    {
        ...
        foreach ($internal_fields as $field_id => $val) {
			...
			if ( isset($val["is_null"]) ) {
                $res .= $val["is_null"] ? " IS NULL " : " IS NOT NULL "; 		//ABR
                
            } elseif ((!isset($val['like'])) || ($val['like'] == 'off')) {
    ...
    function getUsersByOrg($id_org, $only_valid = false, $full_info = false){
		//>> ABR: Restituisce un array con gli utenti di uno specifico nodo dell'organigramma
		//	 L'array include anche gli utenti dei sottonodi
		//	 id_org è l'id dell'organigramma	
		//   Nel model UsermanagementAdm c'è getFolderUsers che restituisce i dati in modo analogo (effettua più chiamate al DB). 
		
		$res = array();	
		$fields = $full_info ? "*" : "idst";
		
		// Preparo Query
		$query_idst = $this->getSqlUsersByOrg($id_org, $only_valid);
		
		$query = "SELECT ".$fields." FROM %adm_user WHERE idst IN (".$query_idst.")";
		...
		if ($full_info) {
			// Sono richieste tutte le informazioni dell'utente
			...
			// E' richiesta solo la sua chiave
			while($row = sql_fetch_assoc($result)) 
			{
				$res[] = $row['idst'];
			}	
	...
	function getSqlUsersByOrg($id_org, $only_valid = false){
		//>> ABR: Restituisce la stringa SQL per il recupero degli utenti di uno specifico nodo dell'organigramma
		//	 La query include anche gli utenti dei sottonodi
		//	 id_org è l'id dell'organigramma
			
		
		// Preparo la query
		$where = !$only_valid ? '' : " AND usr.valid = 1";
		
		$query = "	SELECT usr.idst
					FROM %adm_user usr
						JOIN %adm_group_members grp ON usr.idst = grp.idstMember
						JOIN (select path, idst_oc from %adm_org_chart_tree) sorg ON grp.idst = sorg.idst_oc
						JOIN %adm_org_chart_tree org ON LEFT(sorg.path, CHAR_LENGTH(org.path)) = org.path
						WHERE idOrg = ".(int)$id_org . $where;
    ...
    function getMateUsers($id_user) {
		//>> ABR: Restituisce gli utenti appartenenti ai nodi o sotto-nodi dell'utente passato in argomento
		
		// Prendo i nodi dell'organigramma (escluso root)
		$nodes =  $acl_man->getGroupsIdstFromBasePath('/oc\_');
		$nodes = array_diff($nodes,  $acl_man->getGroupsIdstFromBasePath('/oc_0'));
		
		// Recupero i gruppi di appartenenza dell'utente
		$user_groups = $acl_man->getGroupsContainer($id_user);
		
		// Recupero i nodi a cui appartiene l'utente
		$user_nodes =  array_intersect($nodes, $user_groups);
		
		// Recupero gli utenti appartenenti al nodo dell'utente
		$users = array();
		
		foreach($user_nodes as $idst) {
			$users = array_merge($users, $acl_man->getGroupAllUser($idst));
		}
...
class GroupMembersDataRetriever extends DataRetriever
{
    ...
    public function getRows($startRow, $numRows)
    {
        $id_filter = FormaLms\lib\Get::req('user_id', DOTY_MIXED, '');
